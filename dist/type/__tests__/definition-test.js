...
import { GraphQLEnumType, GraphQLInputObjectType, GraphQLInterfaceType, GraphQLList, GraphQLNonNull, GraphQLObjectType, GraphQLScalarType, GraphQLUnionType, } from '../definition';
...
describe('Type System: Scalars', () => {
    ...
    it('use parseValue for parsing literals if parseLiteral omitted', () => {
        ...
        expect(scalar.parseLiteral(parseValue('{ foo: { bar: $var } }'), { var: 'baz' })).to.equal('parseValue: { foo: { bar: "baz" } }');
    });
...
describe('Type System: Objects', () => {
    ...
    it('rejects an Object type with invalid name', () => {
        expect(() => new GraphQLObjectType({ name: 'bad-name', fields: {} })).to.throw('Names must only contain [_a-zA-Z0-9] but "bad-name" does not.');
    });
    ...
    it('rejects an Object type with incorrectly named fields', () => {
        ...
        expect(() => objType.getFields()).to.throw('Names must only contain [_a-zA-Z0-9] but "bad-name" does not.');
    });
...
describe('Type System: Interfaces', () => {
    ...
    it('rejects an Interface type with invalid name', () => {
        expect(() => new GraphQLInterfaceType({ name: 'bad-name', fields: {} })).to.throw('Names must only contain [_a-zA-Z0-9] but "bad-name" does not.');
    });
...
describe('Type System: Unions', () => {
    ...
    it('rejects an Union type with invalid name', () => {
        expect(() => new GraphQLUnionType({ name: 'bad-name', types: [] })).to.throw('Names must only contain [_a-zA-Z0-9] but "bad-name" does not.');
    });
...
describe('Type System: Enums', () => {
    ...
    it('rejects an Enum type with invalid name', () => {
        expect(() => new GraphQLEnumType({ name: 'bad-name', values: {} })).to.throw('Names must only contain [_a-zA-Z0-9] but "bad-name" does not.');
    });
...
describe('Type System: Input Objects', () => {
    describe('Input Objects must have fields', () => {
        ...
        it('rejects an Input Object type with invalid name', () => {
            expect(() => new GraphQLInputObjectType({ name: 'bad-name', fields: {} })).to.throw('Names must only contain [_a-zA-Z0-9] but "bad-name" does not.');
        });
        ...
        it('rejects an Input Object type with incorrectly named fields', () => {
            ...
            expect(() => inputObjType.getFields()).to.throw('Names must only contain [_a-zA-Z0-9] but "bad-name" does not.');
        });
    });
    ...
    it('Deprecation reason is preserved on fields', () => {
        ...
        expect(inputObjType.toConfig()).to.have.nested.property('fields.deprecatedField.deprecationReason', 'not used anymore');
    });
});
...
describe('Type System: test utility methods', () => {
    ...
    it('JSON.stringifies types', () => {
        ...
        expect(JSON.stringify(new GraphQLList(ListOfScalarsType))).to.equal('"[[Scalar]]"');
    });
