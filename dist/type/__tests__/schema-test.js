...
import { GraphQLInputObjectType, GraphQLInterfaceType, GraphQLList, GraphQLObjectType, GraphQLScalarType, } from '../definition';
    ...
    describe('Validity', () => {
        ...
        describe('A Schema must contain uniquely named types', () => {
            it('rejects a Schema which redefines a built-in type', () => {
                ...
                expect(() => new GraphQLSchema({ query: QueryType })).to.throw('Schema must contain uniquely named types but contains multiple types named "String".');
            });
            it('rejects a Schema when a provided type has no name', () => {
                ...
                const types = [{}, query, {}];
                // @ts-expect-error
                expect(() => new GraphQLSchema({ query, types })).to.throw('One of the provided types for building the Schema is missing a name.');
            });
            it('rejects a Schema which defines an object type twice', () => {
                ...
                expect(() => new GraphQLSchema({ types })).to.throw('Schema must contain uniquely named types but contains multiple types named "SameName".');
            });
            it('rejects a Schema which defines fields with conflicting types', () => {
                ...
                expect(() => new GraphQLSchema({ query: QueryType })).to.throw('Schema must contain uniquely named types but contains multiple types named "SameName".');
            });
        });
