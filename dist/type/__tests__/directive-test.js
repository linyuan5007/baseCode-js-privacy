...
describe('Type System: Directive', () => {
    ...
    it('rejects a directive with undefined locations', () => {
        // @ts-expect-error
        expect(() => new GraphQLDirective({ name: 'Foo' })).to.throw('@Foo locations must be an Array.');
    });
    it('rejects a directive with incorrectly typed locations', () => {
        // @ts-expect-error
        expect(() => new GraphQLDirective({ name: 'Foo', locations: {} })).to.throw('@Foo locations must be an Array.');
    });
});
