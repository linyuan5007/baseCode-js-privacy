...
describe('Subscription Initialization Phase', () => {
    ...
    it('throws an error if some of required arguments are missing', async () => {
        ...
        (await expectPromise(subscribe({ schema: null, document }))).toRejectWith('Expected null to be a GraphQL schema.');
        // @ts-expect-error
        (await expectPromise(subscribe({ document }))).toRejectWith('Expected undefined to be a GraphQL schema.');
        // @ts-expect-error (document must not be null)
        (await expectPromise(subscribe({ schema, document: null }))).toRejectWith('Must provide document.');
        // @ts-expect-error
        (await expectPromise(subscribe({ schema }))).toRejectWith('Must provide document.');
    });
    ...
    it('Deprecated: throws an error if document is missing when using positional arguments', async () => {
        ...
        // @ts-expect-error (schema must not be null)
        (await expectPromise(createSourceEventStream(null, document))).toRejectWith('Expected null to be a GraphQL schema.');
        (await expectPromise(createSourceEventStream(
        // @ts-expect-error
        undefined, document))).toRejectWith('Expected undefined to be a GraphQL schema.');
        // @ts-expect-error (document must not be null)
        (await expectPromise(createSourceEventStream(schema, null))).toRejectWith('Must provide document.');
        // @ts-expect-error
        (await expectPromise(createSourceEventStream(schema))).toRejectWith('Must provide document.');
    });
    ...
    it('throws an error if subscribe does not return an iterator', async () => {
        ...
        const document = parse('subscription { foo }');
        (await expectPromise(subscribe({ schema, document }))).toRejectWith('Subscription field must return Async Iterable. Received: "test".');
    });
    it('resolves to an error for subscription resolver errors', async () => {
        async function subscribeWithFn(subscribeFn) {
            ...
            const document = parse('subscription { foo }');
            const result = await subscribe({ schema, document });
            expectJSON(await createSourceEventStream(schema, document)).toDeepEqual(result);
            return result;
        }
