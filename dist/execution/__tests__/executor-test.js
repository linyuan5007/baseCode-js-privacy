...
import { GraphQLInputObjectType, GraphQLInterfaceType, GraphQLList, GraphQLNonNull, GraphQLObjectType, GraphQLScalarType, GraphQLUnionType, } from '../../type/definition';
...
describe('Execute: Handles basic execution tasks', () => {
    ...
    it('throws if no schema is provided', () => {
        ...
        // @ts-expect-error
        expect(() => executeSync({ document })).to.throw('Expected undefined to be a GraphQL schema.');
    });
    ...
    `);
        ...
        // @ts-expect-error
        expect(() => executeSync({ schema, document, variableValues })).to.throw('Variables must be provided as an Object where each property is a variable value. Perhaps look to see if an unparsed JSON string was provided.');
    });
    ...
    `);
        ...
        expectJSON(result).toDeepEqual({
            errors: [
                {
                    message: 'Variable "$data" got invalid value { email: "", wrongArg: "wrong", wrongArg2: "wrong", wrongArg3: "wrong" }; Field "wrongArg" is not defined by type "User".',
