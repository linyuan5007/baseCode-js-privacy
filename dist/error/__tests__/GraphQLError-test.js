...
describe('toString', () => {
    ...
    it('prints an error using node without location', () => {
        ...
        expect(error.toString()).to.equal('Error attached to node without location');
    });
...
describe('toJSON', () => {
    ...
    it('can be created with the legacy argument list', () => {
        const error = new GraphQLError('msg', [operationNode], source, [6], ['path', 2, 'a'], new Error('I like turtles'), { hee: 'I like turtles' });
        expect(error.toJSON()).to.deep.equal({
            message: 'msg',
