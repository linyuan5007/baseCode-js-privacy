...
function expectSchemaChanges(schema, extendedSchema) {
    ...
    return expect(parse(printSchema(extendedSchema))
...
describe('extendSchema', () => {
    ...
    `);
        expect(() => extendSchema(schema, ast, { assumeValidSDL: true })).to.throw('Unknown type: "UnknownType".');
    });
    it('Rejects invalid AST', () => {
        const schema = new GraphQLSchema({});
        // @ts-expect-error (Second argument expects DocumentNode)
        expect(() => extendSchema(schema, null)).to.throw('Must provide valid Document AST');
        // @ts-expect-error
