...
describe('resolveSchemaCoordinate', () => {
    ...
    it('resolves a Type Field', () => {
        ...
        expect(resolveSchemaCoordinate(schema, 'Business.unknown')).to.deep.equal(undefined);
        expect(() => resolveSchemaCoordinate(schema, 'Unknown.field')).to.throw('Expected "Unknown" to be defined as a type in the schema.');
        expect(() => resolveSchemaCoordinate(schema, 'String.field')).to.throw('Expected "String" to be an Enum, Input Object, Object or Interface type.');
    });
    it('resolves a Input Field', () => {
        const type = schema.getType('SearchCriteria');
        const inputField = type.getFields().filter;
        expect(resolveSchemaCoordinate(schema, 'SearchCriteria.filter')).to.deep.equal({
            kind: 'InputField',
            type,
            inputField,
        });
        expect(resolveSchemaCoordinate(schema, 'SearchCriteria.unknown')).to.deep.equal(undefined);
    });
    it('resolves a Enum Value', () => {
        const type = schema.getType('SearchFilter');
        const enumValue = type.getValue('OPEN_NOW');
        expect(resolveSchemaCoordinate(schema, 'SearchFilter.OPEN_NOW')).to.deep.equal({
            kind: 'EnumValue',
            type,
            enumValue,
        });
        expect(resolveSchemaCoordinate(schema, 'SearchFilter.UNKNOWN')).to.deep.equal(undefined);
    });
    it('resolves a Field Argument', () => {
        ...
        expect(resolveSchemaCoordinate(schema, 'Query.searchBusiness(criteria:)')).to.deep.equal({
            kind: 'FieldArgument',
            type,
            field,
            fieldArgument,
        });
        expect(resolveSchemaCoordinate(schema, 'Business.name(unknown:)')).to.deep.equal(undefined);
        expect(() => resolveSchemaCoordinate(schema, 'Unknown.field(arg:)')).to.throw('Expected "Unknown" to be defined as a type in the schema.');
        expect(() => resolveSchemaCoordinate(schema, 'Business.unknown(arg:)')).to.throw('Expected "unknown" to exist as a field of type "Business" in the schema.');
        expect(() => resolveSchemaCoordinate(schema, 'SearchCriteria.name(arg:)')).to.throw('Expected "SearchCriteria" to be an object type or interface type.');
    });
    it('resolves a Directive', () => {
        ...
        expect(resolveSchemaCoordinate(schema, '@unknown')).to.deep.equal(undefined);
        expect(resolveSchemaCoordinate(schema, '@Business')).to.deep.equal(undefined);
    });
    it('resolves a Directive Argument', () => {
        ...
        expect(resolveSchemaCoordinate(schema, '@private(unknown:)')).to.deep.equal(undefined);
        expect(() => resolveSchemaCoordinate(schema, '@unknown(arg:)')).to.throw('Expected "unknown" to be defined as a directive in the schema.');
    });
});
