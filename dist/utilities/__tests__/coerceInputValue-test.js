...
describe('coerceInputValue', () => {
    ...
    describe('for GraphQLInputObject', () => {
        ...
        it('returns an error for an array type on a nested field', () => {
            const result = coerceValue({ foo: 1, deepObject: [1, 2, 3] }, TestInputObject);
            expectErrors(result).to.deep.equal([
                {
    ...
    describe('for GraphQLInputObject that isOneOf', () => {
        ...
        it('returns an error if more than one field is specified', () => {
            ...
            expectErrors(result).to.deep.equal([
                {
                    error: 'Exactly one key must be specified for OneOf type "TestInputObject".',
                    path: [],
    ...
    describe('for GraphQLInputObject with default value', () => {
        const makeTestInputObject = (defaultValue) => new GraphQLInputObjectType({
            name: 'TestInputObject',
            fields: {
                foo: {
    ...
    describe('for GraphQLList', () => {
        ...
        it('returns a list for a non-list object value', () => {
            const TestListOfObjects = new GraphQLList(new GraphQLInputObjectType({
                name: 'TestObject',
                fields: {
    ...
    describe('with default onError', () => {
        it('throw error without path', () => {
            expect(() => coerceInputValue(null, new GraphQLNonNull(GraphQLInt))).to.throw('Invalid value null: Expected non-nullable type "Int!" not to be null.');
        });
        it('throw error with path', () => {
            expect(() => coerceInputValue([null], new GraphQLList(new GraphQLNonNull(GraphQLInt)))).to.throw('Invalid value null at "value[0]": Expected non-nullable type "Int!" not to be null.');
        });
    });
});
