...
describe('Printer: Query document', () => {
    ...
    it('Legacy: prints fragment with variable directives', () => {
        const queryASTWithVariableDirective = parse('fragment Foo($foo: TestType @test) on TestType @testDirective { id }', { allowLegacyFragmentVariables: true });
        expect(print(queryASTWithVariableDirective)).to.equal(dedent `
    ...
    it('prints fragment', () => {
        const printed = print(parse('"Fragment description" fragment Foo on Bar { baz }'));
        expect(printed).to.equal(dedent `
    ...
    it('prints kitchen sink without altering ast', () => {
        ...
        expect(printed).to.equal(dedentString(String.raw `
      "Query description"
      query queryName(
    ...
    it('prints schema coordinates', () => {
        ...
        expect(print(parseSchemaCoordinate('Name.field(arg:)'))).to.equal('Name.field(arg:)');
        expect(print(parseSchemaCoordinate('@name'))).to.equal('@name');
        expect(print(parseSchemaCoordinate('@name(arg:)'))).to.equal('@name(arg:)');
        expect(print(parseSchemaCoordinate('__Type'))).to.equal('__Type');
        expect(print(parseSchemaCoordinate('Type.__metafield'))).to.equal('Type.__metafield');
        expect(print(parseSchemaCoordinate('Type.__metafield(arg:)'))).to.equal('Type.__metafield(arg:)');
    });
    it('throws syntax error for ignored tokens in schema coordinates', () => {
        ...
        expect(() => print(parseSchemaCoordinate('Name .field'))).to.throw('Syntax Error: Invalid character: " "');
    });
});
