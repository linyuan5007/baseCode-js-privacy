...
`)).to.contain({
    ...
    it('lexes strings', () => {
        ...
        expect(lexOne('"unescaped maximal unicode outside BMP \u{10ffff}"')).to.contain({
            kind: TokenKind.STRING,
            start: 0,
            end: 42,
        ...
        expect(lexOne('"string with unicode escape outside BMP \\u{1F600}"')).to.contain({
            kind: TokenKind.STRING,
            start: 0,
            end: 50,
        ...
        expect(lexOne('"string with maximal unicode escape \\u{10FFFF}"')).to.contain({
            kind: TokenKind.STRING,
            start: 0,
            end: 47,
            value: 'string with maximal unicode escape \u{10FFFF}',
        });
        expect(lexOne('"string with maximal minimal unicode escape \\u{00000000}"')).to.contain({
            kind: TokenKind.STRING,
            start: 0,
            end: 57,
            value: 'string with maximal minimal unicode escape \u{0}',
        });
        expect(lexOne('"string with unicode surrogate pair escape \\uD83D\\uDE00"')).to.contain({
            kind: TokenKind.STRING,
            start: 0,
            end: 56,
            value: 'string with unicode surrogate pair escape \u{1f600}',
        });
        expect(lexOne('"string with minimal surrogate pair escape \\uD800\\uDC00"')).to.contain({
            kind: TokenKind.STRING,
            start: 0,
            end: 56,
            value: 'string with minimal surrogate pair escape \u{10000}',
        });
        expect(lexOne('"string with maximal surrogate pair escape \\uDBFF\\uDFFF"')).to.contain({
            kind: TokenKind.STRING,
            start: 0,
            end: 56,
