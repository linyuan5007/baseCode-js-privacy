...
import { parse, parseConstValue, parseSchemaCoordinate, parseType, parseValue, } from '../parser';
...
describe('Parser', () => {
    ...
    it('limit maximum number of tokens', () => {
        ...
        expect(() => parse('{ foo }', { maxTokens: 2 })).to.throw('Syntax Error: Document contains more that 2 tokens. Parsing aborted.');
        expect(() => parse('{ foo(bar: "baz") }', { maxTokens: 8 })).to.not.throw();
        expect(() => parse('{ foo(bar: "baz") }', { maxTokens: 7 })).to.throw('Syntax Error: Document contains more that 7 tokens. Parsing aborted.');
    });
    ...
    describe('parseSchemaCoordinate', () => {
        ...
        it('accepts a Source object', () => {
            expect(parseSchemaCoordinate('MyType')).to.deep.equal(parseSchemaCoordinate(new Source('MyType')));
        });
    });
    describe('operation and variable definition descriptions', () => {
      ...
      `);
            ...
            const opDef = result.definitions.find((d) => d.kind === Kind.OPERATION_DEFINITION);
        ...
        it('descriptions on a short-hand query produce a sensible error', () => {
            const input = `"""Invalid"""
        { __typename }`;
            expect(() => parse(input)).to.throw('Syntax Error: Unexpected description, descriptions are not supported on shorthand queries.');
        });
      ...
      `);
            ...
            const varDef = (_a = opDef.variableDefinitions) === null || _a === void 0 ? void 0 : _a[0];
        ...
        it('parses fragment with variable description (legacy)', () => {
            ...
            const fragDef = result.definitions.find((d) => d.kind === Kind.FRAGMENT_DEFINITION);
            if (!fragDef || fragDef.kind !== Kind.FRAGMENT_DEFINITION) {
                throw new Error('No fragment definition found');
            }
            const varDef = (_a = fragDef.variableDefinitions) === null || _a === void 0 ? void 0 : _a[0];
            expectJSON(varDef).toDeepEqual({
                kind: Kind.VARIABLE_DEFINITION,
                description: {
                    kind: Kind.STRING,
