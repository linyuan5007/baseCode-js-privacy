    ...
    getFragment(name) {
        ...
        if (this._fragments) {
            ...
            for (const defNode of this.getDocument().definitions) {
                if (defNode.kind === kinds_1.Kind.FRAGMENT_DEFINITION) {
...
class ValidationContext extends ASTValidationContext {
    ...
    getVariableUsages(node) {
        ...
        if (!usages) {
            const newUsages = [];
            const typeInfo = new TypeInfo_1.TypeInfo(this._schema);
            (0, visitor_1.visit)(node, (0, TypeInfo_1.visitWithTypeInfo)(typeInfo, {
