...
function MaxIntrospectionDepthRule(context) {
    ...
    function checkDepth(node, visitedFragments = Object.create(null), depth = 0) {
        if (node.kind === kinds_1.Kind.FRAGMENT_SPREAD) {
        ...
        if (node.kind === kinds_1.Kind.FIELD &&
    ...
    return {
        Field(node) {
            if (node.name.value === '__schema' || node.name.value === '__type') {
                if (checkDepth(node)) {
                    context.reportError(new GraphQLError_1.GraphQLError('Maximum introspection depth exceeded', {
                        nodes: [node],
                    }));
                    return false;
                }
