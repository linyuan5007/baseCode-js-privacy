...
const sortValueNode_1 = require('../../utilities/sortValueNode');
    ...
    return {
        SelectionSet(selectionSet) {
            ...
            for (const [[responseName, reason], fields1, fields2] of conflicts) {
                ...
                context.reportError(new GraphQLError_1.GraphQLError(`Fields "${responseName}" conflict because ${reasonMsg}. Use different aliases on the fields to fetch both if this was intentional.`, { nodes: fields1.concat(fields2) }));
            }
        },
    };
}
...
function findConflict(context, cachedFieldsAndFragmentNames, comparedFieldsAndFragmentPairs, comparedFragmentPairs, parentFieldsAreMutuallyExclusive, responseName, field1, field2) {
    ...
    const areMutuallyExclusive = parentFieldsAreMutuallyExclusive ||
        (parentType1 !== parentType2 &&
            (0, definition_1.isObjectType)(parentType1) &&
            (0, definition_1.isObjectType)(parentType2));
    ...
    if (type1 && type2 && doTypesConflict(type1, type2)) {
        return [
            [
                responseName,
                `they return conflicting types "${(0, inspect_1.inspect)(type1)}" and "${(0, inspect_1.inspect)(type2)}"`,
            ],
            [node1],
            [node2],
        ];
    ...
    if (selectionSet1 && selectionSet2) {
        const conflicts = findConflictsBetweenSubSelectionSets(context, cachedFieldsAndFragmentNames, comparedFieldsAndFragmentPairs, comparedFragmentPairs, areMutuallyExclusive, (0, definition_1.getNamedType)(type1), selectionSet1, (0, definition_1.getNamedType)(type2), selectionSet2);
...
function stringifyValue(value) {
    return (0, printer_1.print)((0, sortValueNode_1.sortValueNode)(value));
}
// Two types conflict if both types could not apply to a value simultaneously.
// Composite types are ignored as their individual field types will be compared
// later recursively. However List and Non-Null types must match.
function doTypesConflict(type1, type2) {
    if ((0, definition_1.isListType)(type1)) {
        return (0, definition_1.isListType)(type2)
            ? doTypesConflict(type1.ofType, type2.ofType)
            : true;
    }
    if ((0, definition_1.isListType)(type2)) {
        return true;
    }
    if ((0, definition_1.isNonNullType)(type1)) {
        return (0, definition_1.isNonNullType)(type2)
            ? doTypesConflict(type1.ofType, type2.ofType)
            : true;
    }
    if ((0, definition_1.isNonNullType)(type2)) {
        return true;
    }
    if ((0, definition_1.isLeafType)(type1) || (0, definition_1.isLeafType)(type2)) {
        return type1 !== type2;
    }
    return false;
}
...
function getReferencedFieldsAndFragmentNames(context, cachedFieldsAndFragmentNames, fragment) {
    ...
    const fragmentType = (0, typeFromAST_1.typeFromAST)(context.getSchema(), fragment.typeCondition);
        ...
        switch (selection.kind) {
            ...
            case kinds_1.Kind.INLINE_FRAGMENT: {
                ...
                const inlineFragmentType = typeCondition
                    ? (0, typeFromAST_1.typeFromAST)(context.getSchema(), typeCondition)
                    : parentType;
