...
function validateOneOfInputObject(context, node, type, fieldNodeMap) {
    ...
    if (isNotExactlyOneField) {
        context.reportError(new GraphQLError_1.GraphQLError(`OneOf Input Object "${type.name}" must specify exactly one key.`, { nodes: [node] }));
        return;
    }
    const value = fieldNodeMap[keys[0]]?.value;
    const isNullLiteral = !value || value.kind === kinds_1.Kind.NULL;
    if (isNullLiteral) {
        context.reportError(new GraphQLError_1.GraphQLError(`Field "${type.name}.${keys[0]}" must be non-null.`, {
            nodes: [node],
        }));
    }
}
