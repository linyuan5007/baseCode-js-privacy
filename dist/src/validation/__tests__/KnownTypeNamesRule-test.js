...
function expectErrors(queryStr) {
    return (0, harness_1.expectValidationErrors)(KnownTypeNamesRule_1.KnownTypeNamesRule, queryStr);
}
function expectErrorsWithSchema(schema, queryStr) {
    return (0, harness_1.expectValidationErrorsWithSchema)(schema, KnownTypeNamesRule_1.KnownTypeNamesRule, queryStr);
}
function expectValid(queryStr) {
    expectErrors(queryStr).toDeepEqual([]);
}
function expectSDLErrors(sdlStr, schema) {
    return (0, harness_1.expectSDLValidationErrors)(schema, KnownTypeNamesRule_1.KnownTypeNamesRule, sdlStr);
}
function expectValidSDL(sdlStr, schema) {
    expectSDLErrors(sdlStr, schema).toDeepEqual([]);
}
(0, mocha_1.describe)('Validate: Known type names', () => {
    (0, mocha_1.it)('known type names are valid', () => {
        expectValid(`
      query Foo(
        $var: String
        $required: [Int!]!
    ...
    (0, mocha_1.it)('unknown type names are invalid', () => {
        expectErrors(`
    ...
    (0, mocha_1.it)('references to standard scalars that are missing in schema', () => {
        const schema = (0, buildASTSchema_1.buildSchema)('type Query { foo: String }');
        const query = `
    ...
    (0, mocha_1.describe)('within SDL', () => {
        (0, mocha_1.it)('use standard types', () => {
            expectValidSDL(`
        type Query {
          string: String
          int: Int
      ...
      `);
        ...
        (0, mocha_1.it)('reference types defined inside the same document', () => {
            expectValidSDL(`
      ...
      `);
        ...
        (0, mocha_1.it)('unknown type references', () => {
            expectSDLErrors(`
        type A
        type B
      ...
      `).toDeepEqual([
        ...
        (0, mocha_1.it)('reference standard types inside extension document', () => {
      ...
      `;
        ...
        (0, mocha_1.it)('reference types inside extension document', () => {
            const schema = (0, buildASTSchema_1.buildSchema)('type Foo');
            const sdl = `
        type QueryRoot {
          foo: Foo
      ...
      `;
        ...
        (0, mocha_1.it)('unknown type references inside extension document', () => {
            const schema = (0, buildASTSchema_1.buildSchema)('type A');
            const sdl = `
        type B
