...
const SearchableInterface = new definition_1.GraphQLInterfaceType({
    name: 'Searchable',
    fields: {
        id: { type: scalars_1.GraphQLString },
    },
});
const TypeA = new definition_1.GraphQLObjectType({
    name: 'TypeA',
    interfaces: [SearchableInterface],
    fields: () => ({
        id: { type: scalars_1.GraphQLString },
        nameA: { type: scalars_1.GraphQLString },
    }),
    isTypeOf: (_value, _context, _info) => new Promise((_resolve, reject) => 
    // eslint-disable-next-line
    setTimeout(() => reject(new Error('TypeA_isTypeOf_rejected')), 10)),
});
const TypeB = new definition_1.GraphQLObjectType({
    name: 'TypeB',
    interfaces: [SearchableInterface],
    fields: () => ({
        id: { type: scalars_1.GraphQLString },
        nameB: { type: scalars_1.GraphQLString },
    }),
    isTypeOf: (value, _context, _info) => value.id === 'b',
});
const queryTypeWithSearchable = new definition_1.GraphQLObjectType({
    name: 'Query',
    fields: {
        person: {
            type: PersonType,
...
const schemaWithSearchable = new schema_1.GraphQLSchema({
...
(0, mocha_1.describe)('Execute: Union and intersection types', () => {
    ...
    (0, mocha_1.it)('handles promises from isTypeOf correctly when a later type matches synchronously', async () => {
        const document = (0, parser_1.parse)(`
      query TestSearch {
        search(id: "b") {
          __typename
    ...
    `);
        ...
        (0, chai_1.expect)(result.errors).to.equal(undefined);
        ...
        (0, chai_1.expect)(unhandledRejection).to.equal(null);
    });
});
