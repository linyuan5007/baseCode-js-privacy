...
(0, mocha_1.describe)('Execute: Handles basic execution tasks', () => {
    ...
    (0, mocha_1.it)('uses a different number of max coercion errors', () => {
        const schema = new schema_1.GraphQLSchema({
            query: new definition_1.GraphQLObjectType({
                name: 'Query',
                fields: {
                        ...
                        args: {
                            data: {
                                type: new definition_1.GraphQLInputObjectType({
                                    name: 'User',
                                    fields: {
                                        email: { type: new definition_1.GraphQLNonNull(scalars_1.GraphQLString) },
                                    },
                                }),
                            },
                        },
    ...
    `);
        ...
        (0, chai_1.expect)(result.errors).to.have.lengthOf(options.maxCoercionErrors + 1);
        (0, expectJSON_1.expectJSON)(result).toDeepEqual({
            errors: [
                {
