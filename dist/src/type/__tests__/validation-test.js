'use strict';
Object.defineProperty(exports, '__esModule', { value: true });
const chai_1 = require('chai');
const mocha_1 = require('mocha');

const dedent_1 = require('../../__testUtils__/dedent');
const expectJSON_1 = require('../../__testUtils__/expectJSON');
const inspect_1 = require('../../jsutils/inspect');
const directiveLocation_1 = require('../../language/directiveLocation');
const parser_1 = require('../../language/parser');
const buildASTSchema_1 = require('../../utilities/buildASTSchema');
const extendSchema_1 = require('../../utilities/extendSchema');
const definition_1 = require('../definition');
const directives_1 = require('../directives');
const scalars_1 = require('../scalars');
const schema_1 = require('../schema');
const validate_1 = require('../validate');

const SomeSchema = (0, buildASTSchema_1.buildSchema)(`
  scalar SomeScalar

  interface SomeInterface { f: SomeObject }

  type SomeObject implements SomeInterface { f: SomeObject }

  union SomeUnion = SomeObject

  enum SomeEnum { ONLY }

  input SomeInputObject { val: String = "hello" }

  directive @SomeDirective on QUERY
`);
const SomeScalarType = (0, definition_1.assertScalarType)(SomeSchema.getType('SomeScalar'));
const SomeInterfaceType = (0, definition_1.assertInterfaceType)(SomeSchema.getType('SomeInterface'));
const SomeObjectType = (0, definition_1.assertObjectType)(SomeSchema.getType('SomeObject'));
const SomeUnionType = (0, definition_1.assertUnionType)(SomeSchema.getType('SomeUnion'));
const SomeEnumType = (0, definition_1.assertEnumType)(SomeSchema.getType('SomeEnum'));
const SomeInputObjectType = (0, definition_1.assertInputObjectType)(SomeSchema.getType('SomeInputObject'));
const SomeDirective = (0, directives_1.assertDirective)(SomeSchema.getDirective('SomeDirective'));
function withModifiers(type) {
    return [
        type,
        new definition_1.GraphQLList(type),
        new definition_1.GraphQLNonNull(type),
        new definition_1.GraphQLNonNull(new definition_1.GraphQLList(type)),
    ];
}
const outputTypes = [
    ...withModifiers(scalars_1.GraphQLString),
    ...withModifiers(SomeScalarType),
    ...withModifiers(SomeEnumType),
    ...withModifiers(SomeObjectType),
    ...withModifiers(SomeUnionType),
    ...withModifiers(SomeInterfaceType),
];
const notOutputTypes = [
    ...withModifiers(SomeInputObjectType),
];
const inputTypes = [
    ...withModifiers(scalars_1.GraphQLString),
    ...withModifiers(SomeScalarType),
    ...withModifiers(SomeEnumType),
    ...withModifiers(SomeInputObjectType),
];
const notInputTypes = [
    ...withModifiers(SomeObjectType),
    ...withModifiers(SomeUnionType),
    ...withModifiers(SomeInterfaceType),
];
function schemaWithFieldType(type) {
    return new schema_1.GraphQLSchema({
        query: new definition_1.GraphQLObjectType({
            name: 'Query',
            fields: { f: { type } },
        }),
    });
}
(0, mocha_1.describe)('Type System: A Schema must have Object root types', () => {
    (0, mocha_1.it)('accepts a Schema whose query type is an object type', () => {
        const schema = (0, buildASTSchema_1.buildSchema)(`
      type Query {
        test: String
      }
    `);
        (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(schema)).toDeepEqual([]);
        const schemaWithDef = (0, buildASTSchema_1.buildSchema)(`
      schema {
        query: QueryRoot
      }

      type QueryRoot {
        test: String
      }
    `);
        (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(schemaWithDef)).toDeepEqual([]);
    });
    (0, mocha_1.it)('accepts a Schema whose query and mutation types are object types', () => {
        const schema = (0, buildASTSchema_1.buildSchema)(`
      type Query {
        test: String
      }

      type Mutation {
        test: String
      }
    `);
        (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(schema)).toDeepEqual([]);
        const schemaWithDef = (0, buildASTSchema_1.buildSchema)(`
      schema {
        query: QueryRoot
        mutation: MutationRoot
      }

      type QueryRoot {
        test: String
      }

      type MutationRoot {
        test: String
      }
    `);
        (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(schemaWithDef)).toDeepEqual([]);
    });
    (0, mocha_1.it)('accepts a Schema whose query and subscription types are object types', () => {
        const schema = (0, buildASTSchema_1.buildSchema)(`
      type Query {
        test: String
      }

      type Subscription {
        test: String
      }
    `);
        (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(schema)).toDeepEqual([]);
        const schemaWithDef = (0, buildASTSchema_1.buildSchema)(`
      schema {
        query: QueryRoot
        subscription: SubscriptionRoot
      }

      type QueryRoot {
        test: String
      }

      type SubscriptionRoot {
        test: String
      }
    `);
        (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(schemaWithDef)).toDeepEqual([]);
    });
    (0, mocha_1.it)('rejects a Schema without a query type', () => {
        const schema = (0, buildASTSchema_1.buildSchema)(`
      type Mutation {
        test: String
      }
    `);
        (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(schema)).toDeepEqual([
            {
                message: 'Query root type must be provided.',
            },
        ]);
        const schemaWithDef = (0, buildASTSchema_1.buildSchema)(`
      schema {
        mutation: MutationRoot
      }

      type MutationRoot {
        test: String
      }
    `);
        (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(schemaWithDef)).toDeepEqual([
            {
                message: 'Query root type must be provided.',
                locations: [{ line: 2, column: 7 }],
            },
        ]);
    });
    (0, mocha_1.it)('rejects a Schema whose query root type is not an Object type', () => {
        const schema = (0, buildASTSchema_1.buildSchema)(`
      input Query {
        test: String
      }
    `);
        (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(schema)).toDeepEqual([
            {
                message: 'Query root type must be Object type, it cannot be Query.',
                locations: [{ line: 2, column: 7 }],
            },
        ]);
        const schemaWithDef = (0, buildASTSchema_1.buildSchema)(`
      schema {
        query: SomeInputObject
      }

      input SomeInputObject {
        test: String
      }
    `);
        (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(schemaWithDef)).toDeepEqual([
            {
                message: 'Query root type must be Object type, it cannot be SomeInputObject.',
                locations: [{ line: 3, column: 16 }],
            },
        ]);
    });
    (0, mocha_1.it)('rejects a Schema whose mutation type is an input type', () => {
        const schema = (0, buildASTSchema_1.buildSchema)(`
      type Query {
        field: String
      }

      input Mutation {
        test: String
      }
    `);
        (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(schema)).toDeepEqual([
            {
                message: 'Mutation root type must be Object type if provided, it cannot be Mutation.',
                locations: [{ line: 6, column: 7 }],
            },
        ]);
        const schemaWithDef = (0, buildASTSchema_1.buildSchema)(`
      schema {
        query: Query
        mutation: SomeInputObject
      }

      type Query {
        field: String
      }

      input SomeInputObject {
        test: String
      }
    `);
        (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(schemaWithDef)).toDeepEqual([
            {
                message: 'Mutation root type must be Object type if provided, it cannot be SomeInputObject.',
                locations: [{ line: 4, column: 19 }],
            },
        ]);
    });
    (0, mocha_1.it)('rejects a Schema whose subscription type is an input type', () => {
        const schema = (0, buildASTSchema_1.buildSchema)(`
      type Query {
        field: String
      }

      input Subscription {
        test: String
      }
    `);
        (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(schema)).toDeepEqual([
            {
                message: 'Subscription root type must be Object type if provided, it cannot be Subscription.',
                locations: [{ line: 6, column: 7 }],
            },
        ]);
        const schemaWithDef = (0, buildASTSchema_1.buildSchema)(`
      schema {
        query: Query
        subscription: SomeInputObject
      }

      type Query {
        field: String
      }

      input SomeInputObject {
        test: String
      }
    `);
        (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(schemaWithDef)).toDeepEqual([
            {
                message: 'Subscription root type must be Object type if provided, it cannot be SomeInputObject.',
                locations: [{ line: 4, column: 23 }],
            },
        ]);
    });
    (0, mocha_1.it)('rejects a Schema extended with invalid root types', () => {
        let schema = (0, buildASTSchema_1.buildSchema)(`
      input SomeInputObject {
        test: String
      }
    `);
        schema = (0, extendSchema_1.extendSchema)(schema, (0, parser_1.parse)(`
        extend schema {
          query: SomeInputObject
        }
      `));
        schema = (0, extendSchema_1.extendSchema)(schema, (0, parser_1.parse)(`
        extend schema {
          mutation: SomeInputObject
        }
      `));
        schema = (0, extendSchema_1.extendSchema)(schema, (0, parser_1.parse)(`
        extend schema {
          subscription: SomeInputObject
        }
      `));
        (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(schema)).toDeepEqual([
            {
                message: 'Query root type must be Object type, it cannot be SomeInputObject.',
                locations: [{ line: 3, column: 18 }],
            },
            {
                message: 'Mutation root type must be Object type if provided, it cannot be SomeInputObject.',
                locations: [{ line: 3, column: 21 }],
            },
            {
                message: 'Subscription root type must be Object type if provided, it cannot be SomeInputObject.',
                locations: [{ line: 3, column: 25 }],
            },
        ]);
    });
    (0, mocha_1.it)('rejects a Schema whose types are incorrectly typed', () => {
        const schema = new schema_1.GraphQLSchema({
            query: SomeObjectType,
            // @ts-expect-error
            types: [{ name: 'SomeType' }, SomeDirective],
        });
        (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(schema)).toDeepEqual([
            {
                message: 'Expected GraphQL named type but got: { name: "SomeType" }.',
            },
            {
                message: 'Expected GraphQL named type but got: @SomeDirective.',
                locations: [{ line: 14, column: 3 }],
            },
        ]);
    });
    (0, mocha_1.it)('rejects a Schema whose directives are incorrectly typed', () => {
        const schema = new schema_1.GraphQLSchema({
            query: SomeObjectType,
            // @ts-expect-error
            directives: [null, 'SomeDirective', SomeScalarType],
        });
        (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(schema)).toDeepEqual([
            {
                message: 'Expected directive but got: null.',
            },
            {
                message: 'Expected directive but got: "SomeDirective".',
            },
            {
                message: 'Expected directive but got: SomeScalar.',
                locations: [{ line: 2, column: 3 }],
            },
        ]);
    });
    (0, mocha_1.it)('rejects a Schema whose directives have empty locations', () => {
        const badDirective = new directives_1.GraphQLDirective({
            name: 'BadDirective',
            args: {},
            locations: [],
        });
        const schema = new schema_1.GraphQLSchema({
            query: SomeObjectType,
            directives: [badDirective],
        });
        (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(schema)).toDeepEqual([
            {
                message: 'Directive @BadDirective must include 1 or more locations.',
            },
        ]);
    });
});
(0, mocha_1.describe)('Type System: Objects must have fields', () => {
    (0, mocha_1.it)('accepts an Object type with fields object', () => {
        const schema = (0, buildASTSchema_1.buildSchema)(`
      type Query {
        field: SomeObject
      }

      type SomeObject {
        field: String
      }
    `);
        (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(schema)).toDeepEqual([]);
    });
    (0, mocha_1.it)('rejects an Object type with missing fields', () => {
        const schema = (0, buildASTSchema_1.buildSchema)(`
      type Query {
        test: IncompleteObject
      }

      type IncompleteObject
    `);
        (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(schema)).toDeepEqual([
            {
                message: 'Type IncompleteObject must define one or more fields.',
                locations: [{ line: 6, column: 7 }],
            },
        ]);
        const manualSchema = schemaWithFieldType(new definition_1.GraphQLObjectType({
            name: 'IncompleteObject',
            fields: {},
        }));
        (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(manualSchema)).toDeepEqual([
            {
                message: 'Type IncompleteObject must define one or more fields.',
            },
        ]);
        const manualSchema2 = schemaWithFieldType(new definition_1.GraphQLObjectType({
            name: 'IncompleteObject',
            fields() {
                return {};
            },
        }));
        (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(manualSchema2)).toDeepEqual([
            {
                message: 'Type IncompleteObject must define one or more fields.',
            },
        ]);
    });
    (0, mocha_1.it)('rejects an Object type with incorrectly named fields', () => {
        const schema = schemaWithFieldType(new definition_1.GraphQLObjectType({
            name: 'SomeObject',
            fields: {
                __badName: { type: scalars_1.GraphQLString },
            },
        }));
        (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(schema)).toDeepEqual([
            {
                message: 'Name "__badName" must not begin with "__", which is reserved by GraphQL introspection.',
            },
        ]);
    });
});
(0, mocha_1.describe)('Type System: Fields args must be properly named', () => {
    (0, mocha_1.it)('accepts field args with valid names', () => {
        const schema = schemaWithFieldType(new definition_1.GraphQLObjectType({
            name: 'SomeObject',
            fields: {
                goodField: {
                    type: scalars_1.GraphQLString,
                    args: {
                        goodArg: { type: scalars_1.GraphQLString },
                    },
                },
            },
        }));
        (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(schema)).toDeepEqual([]);
    });
    (0, mocha_1.it)('rejects field arg with invalid names', () => {
        const schema = schemaWithFieldType(new definition_1.GraphQLObjectType({
            name: 'SomeObject',
            fields: {
                badField: {
                    type: scalars_1.GraphQLString,
                    args: {
                        __badName: { type: scalars_1.GraphQLString },
                    },
                },
            },
        }));
        (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(schema)).toDeepEqual([
            {
                message: 'Name "__badName" must not begin with "__", which is reserved by GraphQL introspection.',
            },
        ]);
    });
});
(0, mocha_1.describe)('Type System: Union types must be valid', () => {
    (0, mocha_1.it)('accepts a Union type with member types', () => {
        const schema = (0, buildASTSchema_1.buildSchema)(`
      type Query {
        test: GoodUnion
      }

      type TypeA {
        field: String
      }

      type TypeB {
        field: String
      }

      union GoodUnion =
        | TypeA
        | TypeB
    `);
        (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(schema)).toDeepEqual([]);
    });
    (0, mocha_1.it)('rejects a Union type with empty types', () => {
        let schema = (0, buildASTSchema_1.buildSchema)(`
      type Query {
        test: BadUnion
      }

      union BadUnion
    `);
        schema = (0, extendSchema_1.extendSchema)(schema, (0, parser_1.parse)(`
        directive @test on UNION

        extend union BadUnion @test
      `));
        (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(schema)).toDeepEqual([
            {
                message: 'Union type BadUnion must define one or more member types.',
                locations: [
                    { line: 6, column: 7 },
                    { line: 4, column: 9 },
                ],
            },
        ]);
    });
    (0, mocha_1.it)('rejects a Union type with duplicated member type', () => {
        let schema = (0, buildASTSchema_1.buildSchema)(`
      type Query {
        test: BadUnion
      }

      type TypeA {
        field: String
      }

      type TypeB {
        field: String
      }

      union BadUnion =
        | TypeA
        | TypeB
        | TypeA
    `);
        (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(schema)).toDeepEqual([
            {
                message: 'Union type BadUnion can only include type TypeA once.',
                locations: [
                    { line: 15, column: 11 },
                    { line: 17, column: 11 },
                ],
            },
        ]);
        schema = (0, extendSchema_1.extendSchema)(schema, (0, parser_1.parse)('extend union BadUnion = TypeB'));
        (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(schema)).toDeepEqual([
            {
                message: 'Union type BadUnion can only include type TypeA once.',
                locations: [
                    { line: 15, column: 11 },
                    { line: 17, column: 11 },
                ],
            },
            {
                message: 'Union type BadUnion can only include type TypeB once.',
                locations: [
                    { line: 16, column: 11 },
                    { line: 1, column: 25 },
                ],
            },
        ]);
    });
    (0, mocha_1.it)('rejects a Union type with non-Object members types', () => {
        let schema = (0, buildASTSchema_1.buildSchema)(`
      type Query {
        test: BadUnion
      }

      type TypeA {
        field: String
      }

      type TypeB {
        field: String
      }

      union BadUnion =
        | TypeA
        | String
        | TypeB
    `);
        schema = (0, extendSchema_1.extendSchema)(schema, (0, parser_1.parse)('extend union BadUnion = Int'));
        (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(schema)).toDeepEqual([
            {
                message: 'Union type BadUnion can only include Object types, it cannot include String.',
                locations: [{ line: 16, column: 11 }],
            },
            {
                message: 'Union type BadUnion can only include Object types, it cannot include Int.',
                locations: [{ line: 1, column: 25 }],
            },
        ]);
        const badUnionMemberTypes = [
            scalars_1.GraphQLString,
            new definition_1.GraphQLNonNull(SomeObjectType),
            new definition_1.GraphQLList(SomeObjectType),
            SomeInterfaceType,
            SomeUnionType,
            SomeEnumType,
            SomeInputObjectType,
        ];
        for (const memberType of badUnionMemberTypes) {
            const badUnion = new definition_1.GraphQLUnionType({
                name: 'BadUnion',
                // @ts-expect-error
                types: [memberType],
            });
            const badSchema = schemaWithFieldType(badUnion);
            (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(badSchema)).toDeepEqual([
                {
                    message: 'Union type BadUnion can only include Object types, ' +
                        `it cannot include ${(0, inspect_1.inspect)(memberType)}.`,
                },
            ]);
        }
    });
});
(0, mocha_1.describe)('Type System: Input Objects must have fields', () => {
    (0, mocha_1.it)('accepts an Input Object type with fields', () => {
        const schema = (0, buildASTSchema_1.buildSchema)(`
      type Query {
        field(arg: SomeInputObject): String
      }

      input SomeInputObject {
        field: String
      }
    `);
        (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(schema)).toDeepEqual([]);
    });
    (0, mocha_1.it)('rejects an Input Object type with missing fields', () => {
        let schema = (0, buildASTSchema_1.buildSchema)(`
      type Query {
        field(arg: SomeInputObject): String
      }

      input SomeInputObject
    `);
        schema = (0, extendSchema_1.extendSchema)(schema, (0, parser_1.parse)(`
        directive @test on INPUT_OBJECT

        extend input SomeInputObject @test
      `));
        (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(schema)).toDeepEqual([
            {
                message: 'Input Object type SomeInputObject must define one or more fields.',
                locations: [
                    { line: 6, column: 7 },
                    { line: 4, column: 9 },
                ],
            },
        ]);
    });
    (0, mocha_1.it)('accepts an Input Object with breakable circular reference', () => {
        const schema = (0, buildASTSchema_1.buildSchema)(`
      type Query {
        field(arg: SomeInputObject): String
      }

      input SomeInputObject {
        self: SomeInputObject
        arrayOfSelf: [SomeInputObject]
        nonNullArrayOfSelf: [SomeInputObject]!
        nonNullArrayOfNonNullSelf: [SomeInputObject!]!
        intermediateSelf: AnotherInputObject
      }

      input AnotherInputObject {
        parent: SomeInputObject
      }
    `);
        (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(schema)).toDeepEqual([]);
    });
    (0, mocha_1.it)('rejects an Input Object with non-breakable circular reference', () => {
        const schema = (0, buildASTSchema_1.buildSchema)(`
      type Query {
        field(arg: SomeInputObject): String
      }

      input SomeInputObject {
        nonNullSelf: SomeInputObject!
      }
    `);
        (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(schema)).toDeepEqual([
            {
                message: 'Cannot reference Input Object "SomeInputObject" within itself through a series of non-null fields: "nonNullSelf".',
                locations: [{ line: 7, column: 9 }],
            },
        ]);
    });
    (0, mocha_1.it)('rejects Input Objects with non-breakable circular reference spread across them', () => {
        const schema = (0, buildASTSchema_1.buildSchema)(`
      type Query {
        field(arg: SomeInputObject): String
      }

      input SomeInputObject {
        startLoop: AnotherInputObject!
      }

      input AnotherInputObject {
        nextInLoop: YetAnotherInputObject!
      }

      input YetAnotherInputObject {
        closeLoop: SomeInputObject!
      }
    `);
        (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(schema)).toDeepEqual([
            {
                message: 'Cannot reference Input Object "SomeInputObject" within itself through a series of non-null fields: "startLoop.nextInLoop.closeLoop".',
                locations: [
                    { line: 7, column: 9 },
                    { line: 11, column: 9 },
                    { line: 15, column: 9 },
                ],
            },
        ]);
    });
    (0, mocha_1.it)('rejects Input Objects with multiple non-breakable circular reference', () => {
        const schema = (0, buildASTSchema_1.buildSchema)(`
      type Query {
        field(arg: SomeInputObject): String
      }

      input SomeInputObject {
        startLoop: AnotherInputObject!
      }

      input AnotherInputObject {
        closeLoop: SomeInputObject!
        startSecondLoop: YetAnotherInputObject!
      }

      input YetAnotherInputObject {
        closeSecondLoop: AnotherInputObject!
        nonNullSelf: YetAnotherInputObject!
      }
    `);
        (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(schema)).toDeepEqual([
            {
                message: 'Cannot reference Input Object "SomeInputObject" within itself through a series of non-null fields: "startLoop.closeLoop".',
                locations: [
                    { line: 7, column: 9 },
                    { line: 11, column: 9 },
                ],
            },
            {
                message: 'Cannot reference Input Object "AnotherInputObject" within itself through a series of non-null fields: "startSecondLoop.closeSecondLoop".',
                locations: [
                    { line: 12, column: 9 },
                    { line: 16, column: 9 },
                ],
            },
            {
                message: 'Cannot reference Input Object "YetAnotherInputObject" within itself through a series of non-null fields: "nonNullSelf".',
                locations: [{ line: 17, column: 9 }],
            },
        ]);
    });
    (0, mocha_1.it)('rejects an Input Object type with incorrectly typed fields', () => {
        const schema = (0, buildASTSchema_1.buildSchema)(`
      type Query {
        field(arg: SomeInputObject): String
      }

      type SomeObject {
        field: String
      }

      union SomeUnion = SomeObject

      input SomeInputObject {
        badObject: SomeObject
        badUnion: SomeUnion
        goodInputObject: SomeInputObject
      }
    `);
        (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(schema)).toDeepEqual([
            {
                message: 'The type of SomeInputObject.badObject must be Input Type but got: SomeObject.',
                locations: [{ line: 13, column: 20 }],
            },
            {
                message: 'The type of SomeInputObject.badUnion must be Input Type but got: SomeUnion.',
                locations: [{ line: 14, column: 19 }],
            },
        ]);
    });
    (0, mocha_1.it)('rejects an Input Object type with required argument that is deprecated', () => {
        const schema = (0, buildASTSchema_1.buildSchema)(`
      type Query {
        field(arg: SomeInputObject): String
      }

      input SomeInputObject {
        badField: String! @deprecated
        optionalField: String @deprecated
        anotherOptionalField: String! = "" @deprecated
      }
    `);
        (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(schema)).toDeepEqual([
            {
                message: 'Required input field SomeInputObject.badField cannot be deprecated.',
                locations: [
                    { line: 7, column: 27 },
                    { line: 7, column: 19 },
                ],
            },
        ]);
    });
});
(0, mocha_1.describe)('Type System: Enum types must be well defined', () => {
    (0, mocha_1.it)('rejects an Enum type without values', () => {
        let schema = (0, buildASTSchema_1.buildSchema)(`
      type Query {
        field: SomeEnum
      }

      enum SomeEnum
    `);
        schema = (0, extendSchema_1.extendSchema)(schema, (0, parser_1.parse)(`
        directive @test on ENUM

        extend enum SomeEnum @test
      `));
        (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(schema)).toDeepEqual([
            {
                message: 'Enum type SomeEnum must define one or more values.',
                locations: [
                    { line: 6, column: 7 },
                    { line: 4, column: 9 },
                ],
            },
        ]);
    });
    (0, mocha_1.it)('rejects an Enum type with incorrectly named values', () => {
        const schema = schemaWithFieldType(new definition_1.GraphQLEnumType({
            name: 'SomeEnum',
            values: {
                __badName: {},
            },
        }));
        (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(schema)).toDeepEqual([
            {
                message: 'Name "__badName" must not begin with "__", which is reserved by GraphQL introspection.',
            },
        ]);
    });
});
(0, mocha_1.describe)('Type System: Object fields must have output types', () => {
    function schemaWithObjectField(fieldConfig) {
        const BadObjectType = new definition_1.GraphQLObjectType({
            name: 'BadObject',
            fields: {
                badField: fieldConfig,
            },
        });
        return new schema_1.GraphQLSchema({
            query: new definition_1.GraphQLObjectType({
                name: 'Query',
                fields: {
                    f: { type: BadObjectType },
                },
            }),
            types: [SomeObjectType],
        });
    }
    for (const type of outputTypes) {
        const typeName = (0, inspect_1.inspect)(type);
        (0, mocha_1.it)(`accepts an output type as an Object field type: ${typeName}`, () => {
            const schema = schemaWithObjectField({ type });
            (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(schema)).toDeepEqual([]);
        });
    }
    (0, mocha_1.it)('rejects an empty Object field type', () => {
        // @ts-expect-error (type field must not be undefined)
        const schema = schemaWithObjectField({ type: undefined });
        (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(schema)).toDeepEqual([
            {
                message: 'The type of BadObject.badField must be Output Type but got: undefined.',
            },
        ]);
    });
    for (const type of notOutputTypes) {
        const typeStr = (0, inspect_1.inspect)(type);
        (0, mocha_1.it)(`rejects a non-output type as an Object field type: ${typeStr}`, () => {
            // @ts-expect-error
            const schema = schemaWithObjectField({ type });
            (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(schema)).toDeepEqual([
                {
                    message: `The type of BadObject.badField must be Output Type but got: ${typeStr}.`,
                },
            ]);
        });
    }
    (0, mocha_1.it)('rejects a non-type value as an Object field type', () => {
        // @ts-expect-error
        const schema = schemaWithObjectField({ type: Number });
        (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(schema)).toDeepEqual([
            {
                message: 'The type of BadObject.badField must be Output Type but got: [function Number].',
            },
            {
                message: 'Expected GraphQL named type but got: [function Number].',
            },
        ]);
    });
    (0, mocha_1.it)('rejects with relevant locations for a non-output type as an Object field type', () => {
        const schema = (0, buildASTSchema_1.buildSchema)(`
      type Query {
        field: [SomeInputObject]
      }

      input SomeInputObject {
        field: String
      }
    `);
        (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(schema)).toDeepEqual([
            {
                message: 'The type of Query.field must be Output Type but got: [SomeInputObject].',
                locations: [{ line: 3, column: 16 }],
            },
        ]);
    });
});
(0, mocha_1.describe)('Type System: Objects can only implement unique interfaces', () => {
    (0, mocha_1.it)('rejects an Object implementing a non-type value', () => {
        const schema = new schema_1.GraphQLSchema({
            query: new definition_1.GraphQLObjectType({
                name: 'BadObject',
                // @ts-expect-error (interfaces must not contain undefined)
                interfaces: [undefined],
                fields: { f: { type: scalars_1.GraphQLString } },
            }),
        });
        (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(schema)).toDeepEqual([
            {
                message: 'Type BadObject must only implement Interface types, it cannot implement undefined.',
            },
        ]);
    });
    (0, mocha_1.it)('rejects an Object implementing a non-Interface type', () => {
        const schema = (0, buildASTSchema_1.buildSchema)(`
      type Query {
        test: BadObject
      }

      input SomeInputObject {
        field: String
      }

      type BadObject implements SomeInputObject {
        field: String
      }
    `);
        (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(schema)).toDeepEqual([
            {
                message: 'Type BadObject must only implement Interface types, it cannot implement SomeInputObject.',
                locations: [{ line: 10, column: 33 }],
            },
        ]);
    });
    (0, mocha_1.it)('rejects an Object implementing the same interface twice', () => {
        const schema = (0, buildASTSchema_1.buildSchema)(`
      type Query {
        test: AnotherObject
      }

      interface AnotherInterface {
        field: String
      }

      type AnotherObject implements AnotherInterface & AnotherInterface {
        field: String
      }
    `);
        (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(schema)).toDeepEqual([
            {
                message: 'Type AnotherObject can only implement AnotherInterface once.',
                locations: [
                    { line: 10, column: 37 },
                    { line: 10, column: 56 },
                ],
            },
        ]);
    });
    (0, mocha_1.it)('rejects an Object implementing the same interface twice due to extension', () => {
        const schema = (0, buildASTSchema_1.buildSchema)(`
      type Query {
        test: AnotherObject
      }

      interface AnotherInterface {
        field: String
      }

      type AnotherObject implements AnotherInterface {
        field: String
      }
    `);
        const extendedSchema = (0, extendSchema_1.extendSchema)(schema, (0, parser_1.parse)('extend type AnotherObject implements AnotherInterface'));
        (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(extendedSchema)).toDeepEqual([
            {
                message: 'Type AnotherObject can only implement AnotherInterface once.',
                locations: [
                    { line: 10, column: 37 },
                    { line: 1, column: 38 },
                ],
            },
        ]);
    });
});
(0, mocha_1.describe)('Type System: Interface extensions should be valid', () => {
    (0, mocha_1.it)('rejects an Object implementing the extended interface due to missing field', () => {
        const schema = (0, buildASTSchema_1.buildSchema)(`
      type Query {
        test: AnotherObject
      }

      interface AnotherInterface {
        field: String
      }

      type AnotherObject implements AnotherInterface {
        field: String
      }
    `);
        const extendedSchema = (0, extendSchema_1.extendSchema)(schema, (0, parser_1.parse)(`
        extend interface AnotherInterface {
          newField: String
        }

        extend type AnotherObject {
          differentNewField: String
        }
      `));
        (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(extendedSchema)).toDeepEqual([
            {
                message: 'Interface field AnotherInterface.newField expected but AnotherObject does not provide it.',
                locations: [
                    { line: 3, column: 11 },
                    { line: 10, column: 7 },
                    { line: 6, column: 9 },
                ],
            },
        ]);
    });
    (0, mocha_1.it)('rejects an Object implementing the extended interface due to missing field args', () => {
        const schema = (0, buildASTSchema_1.buildSchema)(`
      type Query {
        test: AnotherObject
      }

      interface AnotherInterface {
        field: String
      }

      type AnotherObject implements AnotherInterface {
        field: String
      }
    `);
        const extendedSchema = (0, extendSchema_1.extendSchema)(schema, (0, parser_1.parse)(`
        extend interface AnotherInterface {
          newField(test: Boolean): String
        }

        extend type AnotherObject {
          newField: String
        }
      `));
        (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(extendedSchema)).toDeepEqual([
            {
                message: 'Interface field argument AnotherInterface.newField(test:) expected but AnotherObject.newField does not provide it.',
                locations: [
                    { line: 3, column: 20 },
                    { line: 7, column: 11 },
                ],
            },
        ]);
    });
    (0, mocha_1.it)('rejects Objects implementing the extended interface due to mismatching interface type', () => {
        const schema = (0, buildASTSchema_1.buildSchema)(`
      type Query {
        test: AnotherObject
      }

      interface AnotherInterface {
        field: String
      }

      type AnotherObject implements AnotherInterface {
        field: String
      }
    `);
        const extendedSchema = (0, extendSchema_1.extendSchema)(schema, (0, parser_1.parse)(`
        extend interface AnotherInterface {
          newInterfaceField: NewInterface
        }

        interface NewInterface {
          newField: String
        }

        interface MismatchingInterface {
          newField: String
        }

        extend type AnotherObject {
          newInterfaceField: MismatchingInterface
        }

        # Required to prevent unused interface errors
        type DummyObject implements NewInterface & MismatchingInterface {
          newField: String
        }
      `));
        (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(extendedSchema)).toDeepEqual([
            {
                message: 'Interface field AnotherInterface.newInterfaceField expects type NewInterface but AnotherObject.newInterfaceField is type MismatchingInterface.',
                locations: [
                    { line: 3, column: 30 },
                    { line: 15, column: 30 },
                ],
            },
        ]);
    });
});
(0, mocha_1.describe)('Type System: Interface fields must have output types', () => {
    function schemaWithInterfaceField(fieldConfig) {
        const fields = { badField: fieldConfig };
        const BadInterfaceType = new definition_1.GraphQLInterfaceType({
            name: 'BadInterface',
            fields,
        });
        const BadImplementingType = new definition_1.GraphQLObjectType({
            name: 'BadImplementing',
            interfaces: [BadInterfaceType],
            fields,
        });
        return new schema_1.GraphQLSchema({
            query: new definition_1.GraphQLObjectType({
                name: 'Query',
                fields: {
                    f: { type: BadInterfaceType },
                },
            }),
            types: [BadImplementingType, SomeObjectType],
        });
    }
    for (const type of outputTypes) {
        const typeName = (0, inspect_1.inspect)(type);
        (0, mocha_1.it)(`accepts an output type as an Interface field type: ${typeName}`, () => {
            const schema = schemaWithInterfaceField({ type });
            (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(schema)).toDeepEqual([]);
        });
    }
    (0, mocha_1.it)('rejects an empty Interface field type', () => {
        // @ts-expect-error (type field must not be undefined)
        const schema = schemaWithInterfaceField({ type: undefined });
        (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(schema)).toDeepEqual([
            {
                message: 'The type of BadImplementing.badField must be Output Type but got: undefined.',
            },
            {
                message: 'The type of BadInterface.badField must be Output Type but got: undefined.',
            },
        ]);
    });
    for (const type of notOutputTypes) {
        const typeStr = (0, inspect_1.inspect)(type);
        (0, mocha_1.it)(`rejects a non-output type as an Interface field type: ${typeStr}`, () => {
            // @ts-expect-error
            const schema = schemaWithInterfaceField({ type });
            (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(schema)).toDeepEqual([
                {
                    message: `The type of BadImplementing.badField must be Output Type but got: ${typeStr}.`,
                },
                {
                    message: `The type of BadInterface.badField must be Output Type but got: ${typeStr}.`,
                },
            ]);
        });
    }
    (0, mocha_1.it)('rejects a non-type value as an Interface field type', () => {
        // @ts-expect-error
        const schema = schemaWithInterfaceField({ type: Number });
        (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(schema)).toDeepEqual([
            {
                message: 'The type of BadImplementing.badField must be Output Type but got: [function Number].',
            },
            {
                message: 'The type of BadInterface.badField must be Output Type but got: [function Number].',
            },
            {
                message: 'Expected GraphQL named type but got: [function Number].',
            },
        ]);
    });
    (0, mocha_1.it)('rejects a non-output type as an Interface field type with locations', () => {
        const schema = (0, buildASTSchema_1.buildSchema)(`
      type Query {
        test: SomeInterface
      }

      interface SomeInterface {
        field: SomeInputObject
      }

      input SomeInputObject {
        foo: String
      }

      type SomeObject implements SomeInterface {
        field: SomeInputObject
      }
    `);
        (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(schema)).toDeepEqual([
            {
                message: 'The type of SomeInterface.field must be Output Type but got: SomeInputObject.',
                locations: [{ line: 7, column: 16 }],
            },
            {
                message: 'The type of SomeObject.field must be Output Type but got: SomeInputObject.',
                locations: [{ line: 15, column: 16 }],
            },
        ]);
    });
    (0, mocha_1.it)('accepts an interface not implemented by at least one object', () => {
        const schema = (0, buildASTSchema_1.buildSchema)(`
      type Query {
        test: SomeInterface
      }

      interface SomeInterface {
        foo: String
      }
    `);
        (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(schema)).toDeepEqual([]);
    });
});
(0, mocha_1.describe)('Type System: Arguments must have input types', () => {
    function schemaWithArg(argConfig) {
        const BadObjectType = new definition_1.GraphQLObjectType({
            name: 'BadObject',
            fields: {
                badField: {
                    type: scalars_1.GraphQLString,
                    args: {
                        badArg: argConfig,
                    },
                },
            },
        });
        return new schema_1.GraphQLSchema({
            query: new definition_1.GraphQLObjectType({
                name: 'Query',
                fields: {
                    f: { type: BadObjectType },
                },
            }),
            directives: [
                new directives_1.GraphQLDirective({
                    name: 'BadDirective',
                    args: {
                        badArg: argConfig,
                    },
                    locations: [directiveLocation_1.DirectiveLocation.QUERY],
                }),
            ],
        });
    }
    for (const type of inputTypes) {
        const typeName = (0, inspect_1.inspect)(type);
        (0, mocha_1.it)(`accepts an input type as a field arg type: ${typeName}`, () => {
            const schema = schemaWithArg({ type });
            (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(schema)).toDeepEqual([]);
        });
    }
    (0, mocha_1.it)('rejects an empty field arg type', () => {
        // @ts-expect-error (type field must not be undefined)
        const schema = schemaWithArg({ type: undefined });
        (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(schema)).toDeepEqual([
            {
                message: 'The type of @BadDirective(badArg:) must be Input Type but got: undefined.',
            },
            {
                message: 'The type of BadObject.badField(badArg:) must be Input Type but got: undefined.',
            },
        ]);
    });
    for (const type of notInputTypes) {
        const typeStr = (0, inspect_1.inspect)(type);
        (0, mocha_1.it)(`rejects a non-input type as a field arg type: ${typeStr}`, () => {
            // @ts-expect-error
            const schema = schemaWithArg({ type });
            (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(schema)).toDeepEqual([
                {
                    message: `The type of @BadDirective(badArg:) must be Input Type but got: ${typeStr}.`,
                },
                {
                    message: `The type of BadObject.badField(badArg:) must be Input Type but got: ${typeStr}.`,
                },
            ]);
        });
    }
    (0, mocha_1.it)('rejects a non-type value as a field arg type', () => {
        // @ts-expect-error
        const schema = schemaWithArg({ type: Number });
        (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(schema)).toDeepEqual([
            {
                message: 'The type of @BadDirective(badArg:) must be Input Type but got: [function Number].',
            },
            {
                message: 'The type of BadObject.badField(badArg:) must be Input Type but got: [function Number].',
            },
            {
                message: 'Expected GraphQL named type but got: [function Number].',
            },
        ]);
    });
    (0, mocha_1.it)('rejects a required argument that is deprecated', () => {
        const schema = (0, buildASTSchema_1.buildSchema)(`
      directive @BadDirective(
        badArg: String! @deprecated
        optionalArg: String @deprecated
        anotherOptionalArg: String! = "" @deprecated
      ) on FIELD

      type Query {
        test(
          badArg: String! @deprecated
          optionalArg: String @deprecated
          anotherOptionalArg: String! = "" @deprecated
        ): String
      }
    `);
        (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(schema)).toDeepEqual([
            {
                message: 'Required argument @BadDirective(badArg:) cannot be deprecated.',
                locations: [
                    { line: 3, column: 25 },
                    { line: 3, column: 17 },
                ],
            },
            {
                message: 'Required argument Query.test(badArg:) cannot be deprecated.',
                locations: [
                    { line: 10, column: 27 },
                    { line: 10, column: 19 },
                ],
            },
        ]);
    });
    (0, mocha_1.it)('rejects a non-input type as a field arg with locations', () => {
        const schema = (0, buildASTSchema_1.buildSchema)(`
      type Query {
        test(arg: SomeObject): String
      }

      type SomeObject {
        foo: String
      }
    `);
        (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(schema)).toDeepEqual([
            {
                message: 'The type of Query.test(arg:) must be Input Type but got: SomeObject.',
                locations: [{ line: 3, column: 19 }],
            },
        ]);
    });
});
(0, mocha_1.describe)('Type System: Input Object fields must have input types', () => {
    function schemaWithInputField(inputFieldConfig) {
        const BadInputObjectType = new definition_1.GraphQLInputObjectType({
            name: 'BadInputObject',
            fields: {
                badField: inputFieldConfig,
            },
        });
        return new schema_1.GraphQLSchema({
            query: new definition_1.GraphQLObjectType({
                name: 'Query',
                fields: {
                    f: {
                        type: scalars_1.GraphQLString,
                        args: {
                            badArg: { type: BadInputObjectType },
                        },
                    },
                },
            }),
        });
    }
    for (const type of inputTypes) {
        const typeName = (0, inspect_1.inspect)(type);
        (0, mocha_1.it)(`accepts an input type as an input field type: ${typeName}`, () => {
            const schema = schemaWithInputField({ type });
            (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(schema)).toDeepEqual([]);
        });
    }
    (0, mocha_1.it)('rejects an empty input field type', () => {
        // @ts-expect-error (type field must not be undefined)
        const schema = schemaWithInputField({ type: undefined });
        (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(schema)).toDeepEqual([
            {
                message: 'The type of BadInputObject.badField must be Input Type but got: undefined.',
            },
        ]);
    });
    for (const type of notInputTypes) {
        const typeStr = (0, inspect_1.inspect)(type);
        (0, mocha_1.it)(`rejects a non-input type as an input field type: ${typeStr}`, () => {
            // @ts-expect-error
            const schema = schemaWithInputField({ type });
            (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(schema)).toDeepEqual([
                {
                    message: `The type of BadInputObject.badField must be Input Type but got: ${typeStr}.`,
                },
            ]);
        });
    }
    (0, mocha_1.it)('rejects a non-type value as an input field type', () => {
        // @ts-expect-error
        const schema = schemaWithInputField({ type: Number });
        (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(schema)).toDeepEqual([
            {
                message: 'The type of BadInputObject.badField must be Input Type but got: [function Number].',
            },
            {
                message: 'Expected GraphQL named type but got: [function Number].',
            },
        ]);
    });
    (0, mocha_1.it)('rejects a non-input type as an input object field with locations', () => {
        const schema = (0, buildASTSchema_1.buildSchema)(`
      type Query {
        test(arg: SomeInputObject): String
      }

      input SomeInputObject {
        foo: SomeObject
      }

      type SomeObject {
        bar: String
      }
    `);
        (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(schema)).toDeepEqual([
            {
                message: 'The type of SomeInputObject.foo must be Input Type but got: SomeObject.',
                locations: [{ line: 7, column: 14 }],
            },
        ]);
    });
});
(0, mocha_1.describe)('Type System: OneOf Input Object fields must be nullable', () => {
    (0, mocha_1.it)('rejects non-nullable fields', () => {
        const schema = (0, buildASTSchema_1.buildSchema)(`
      type Query {
        test(arg: SomeInputObject): String
      }

      input SomeInputObject @oneOf {
        a: String
        b: String!
      }
    `);
        (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(schema)).toDeepEqual([
            {
                message: 'OneOf input field SomeInputObject.b must be nullable.',
                locations: [{ line: 8, column: 12 }],
            },
        ]);
    });
    (0, mocha_1.it)('rejects fields with default values', () => {
        const schema = (0, buildASTSchema_1.buildSchema)(`
      type Query {
        test(arg: SomeInputObject): String
      }

      input SomeInputObject @oneOf {
        a: String
        b: String = "foo"
      }
    `);
        (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(schema)).toDeepEqual([
            {
                message: 'OneOf input field SomeInputObject.b cannot have a default value.',
                locations: [{ line: 8, column: 9 }],
            },
        ]);
    });
});
(0, mocha_1.describe)('Objects must adhere to Interface they implement', () => {
    (0, mocha_1.it)('accepts an Object which implements an Interface', () => {
        const schema = (0, buildASTSchema_1.buildSchema)(`
      type Query {
        test: AnotherObject
      }

      interface AnotherInterface {
        field(input: String): String
      }

      type AnotherObject implements AnotherInterface {
        field(input: String): String
      }
    `);
        (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(schema)).toDeepEqual([]);
    });
    (0, mocha_1.it)('accepts an Object which implements an Interface along with more fields', () => {
        const schema = (0, buildASTSchema_1.buildSchema)(`
      type Query {
        test: AnotherObject
      }

      interface AnotherInterface {
        field(input: String): String
      }

      type AnotherObject implements AnotherInterface {
        field(input: String): String
        anotherField: String
      }
    `);
        (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(schema)).toDeepEqual([]);
    });
    (0, mocha_1.it)('accepts an Object which implements an Interface field along with additional optional arguments', () => {
        const schema = (0, buildASTSchema_1.buildSchema)(`
      type Query {
        test: AnotherObject
      }

      interface AnotherInterface {
        field(input: String): String
      }

      type AnotherObject implements AnotherInterface {
        field(input: String, anotherInput: String): String
      }
    `);
        (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(schema)).toDeepEqual([]);
    });
    (0, mocha_1.it)('rejects an Object missing an Interface field', () => {
        const schema = (0, buildASTSchema_1.buildSchema)(`
      type Query {
        test: AnotherObject
      }

      interface AnotherInterface {
        field(input: String): String
      }

      type AnotherObject implements AnotherInterface {
        anotherField: String
      }
    `);
        (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(schema)).toDeepEqual([
            {
                message: 'Interface field AnotherInterface.field expected but AnotherObject does not provide it.',
                locations: [
                    { line: 7, column: 9 },
                    { line: 10, column: 7 },
                ],
            },
        ]);
    });
    (0, mocha_1.it)('rejects an Object with an incorrectly typed Interface field', () => {
        const schema = (0, buildASTSchema_1.buildSchema)(`
      type Query {
        test: AnotherObject
      }

      interface AnotherInterface {
        field(input: String): String
      }

      type AnotherObject implements AnotherInterface {
        field(input: String): Int
      }
    `);
        (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(schema)).toDeepEqual([
            {
                message: 'Interface field AnotherInterface.field expects type String but AnotherObject.field is type Int.',
                locations: [
                    { line: 7, column: 31 },
                    { line: 11, column: 31 },
                ],
            },
        ]);
    });
    (0, mocha_1.it)('rejects an Object with a differently typed Interface field', () => {
        const schema = (0, buildASTSchema_1.buildSchema)(`
      type Query {
        test: AnotherObject
      }

      type A { foo: String }
      type B { foo: String }

      interface AnotherInterface {
        field: A
      }

      type AnotherObject implements AnotherInterface {
        field: B
      }
    `);
        (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(schema)).toDeepEqual([
            {
                message: 'Interface field AnotherInterface.field expects type A but AnotherObject.field is type B.',
                locations: [
                    { line: 10, column: 16 },
                    { line: 14, column: 16 },
                ],
            },
        ]);
    });
    (0, mocha_1.it)('accepts an Object with a subtyped Interface field (interface)', () => {
        const schema = (0, buildASTSchema_1.buildSchema)(`
      type Query {
        test: AnotherObject
      }

      interface AnotherInterface {
        field: AnotherInterface
      }

      type AnotherObject implements AnotherInterface {
        field: AnotherObject
      }
    `);
        (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(schema)).toDeepEqual([]);
    });
    (0, mocha_1.it)('accepts an Object with a subtyped Interface field (union)', () => {
        const schema = (0, buildASTSchema_1.buildSchema)(`
      type Query {
        test: AnotherObject
      }

      type SomeObject {
        field: String
      }

      union SomeUnionType = SomeObject

      interface AnotherInterface {
        field: SomeUnionType
      }

      type AnotherObject implements AnotherInterface {
        field: SomeObject
      }
    `);
        (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(schema)).toDeepEqual([]);
    });
    (0, mocha_1.it)('rejects an Object missing an Interface argument', () => {
        const schema = (0, buildASTSchema_1.buildSchema)(`
      type Query {
        test: AnotherObject
      }

      interface AnotherInterface {
        field(input: String): String
      }

      type AnotherObject implements AnotherInterface {
        field: String
      }
    `);
        (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(schema)).toDeepEqual([
            {
                message: 'Interface field argument AnotherInterface.field(input:) expected but AnotherObject.field does not provide it.',
                locations: [
                    { line: 7, column: 15 },
                    { line: 11, column: 9 },
                ],
            },
        ]);
    });
    (0, mocha_1.it)('rejects an Object with an incorrectly typed Interface argument', () => {
        const schema = (0, buildASTSchema_1.buildSchema)(`
      type Query {
        test: AnotherObject
      }

      interface AnotherInterface {
        field(input: String): String
      }

      type AnotherObject implements AnotherInterface {
        field(input: Int): String
      }
    `);
        (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(schema)).toDeepEqual([
            {
                message: 'Interface field argument AnotherInterface.field(input:) expects type String but AnotherObject.field(input:) is type Int.',
                locations: [
                    { line: 7, column: 22 },
                    { line: 11, column: 22 },
                ],
            },
        ]);
    });
    (0, mocha_1.it)('rejects an Object with both an incorrectly typed field and argument', () => {
        const schema = (0, buildASTSchema_1.buildSchema)(`
      type Query {
        test: AnotherObject
      }

      interface AnotherInterface {
        field(input: String): String
      }

      type AnotherObject implements AnotherInterface {
        field(input: Int): Int
      }
    `);
        (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(schema)).toDeepEqual([
            {
                message: 'Interface field AnotherInterface.field expects type String but AnotherObject.field is type Int.',
                locations: [
                    { line: 7, column: 31 },
                    { line: 11, column: 28 },
                ],
            },
            {
                message: 'Interface field argument AnotherInterface.field(input:) expects type String but AnotherObject.field(input:) is type Int.',
                locations: [
                    { line: 7, column: 22 },
                    { line: 11, column: 22 },
                ],
            },
        ]);
    });
    (0, mocha_1.it)('rejects an Object which implements an Interface field along with additional required arguments', () => {
        const schema = (0, buildASTSchema_1.buildSchema)(`
      type Query {
        test: AnotherObject
      }

      interface AnotherInterface {
        field(baseArg: String): String
      }

      type AnotherObject implements AnotherInterface {
        field(
          baseArg: String,
          requiredArg: String!
          optionalArg1: String,
          optionalArg2: String = "",
        ): String
      }
    `);
        (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(schema)).toDeepEqual([
            {
                message: 'Object field AnotherObject.field includes required argument requiredArg that is missing from the Interface field AnotherInterface.field.',
                locations: [
                    { line: 13, column: 11 },
                    { line: 7, column: 9 },
                ],
            },
        ]);
    });
    (0, mocha_1.it)('accepts an Object with an equivalently wrapped Interface field type', () => {
        const schema = (0, buildASTSchema_1.buildSchema)(`
      type Query {
        test: AnotherObject
      }

      interface AnotherInterface {
        field: [String]!
      }

      type AnotherObject implements AnotherInterface {
        field: [String]!
      }
    `);
        (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(schema)).toDeepEqual([]);
    });
    (0, mocha_1.it)('rejects an Object with a non-list Interface field list type', () => {
        const schema = (0, buildASTSchema_1.buildSchema)(`
      type Query {
        test: AnotherObject
      }

      interface AnotherInterface {
        field: [String]
      }

      type AnotherObject implements AnotherInterface {
        field: String
      }
    `);
        (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(schema)).toDeepEqual([
            {
                message: 'Interface field AnotherInterface.field expects type [String] but AnotherObject.field is type String.',
                locations: [
                    { line: 7, column: 16 },
                    { line: 11, column: 16 },
                ],
            },
        ]);
    });
    (0, mocha_1.it)('rejects an Object with a list Interface field non-list type', () => {
        const schema = (0, buildASTSchema_1.buildSchema)(`
      type Query {
        test: AnotherObject
      }

      interface AnotherInterface {
        field: String
      }

      type AnotherObject implements AnotherInterface {
        field: [String]
      }
    `);
        (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(schema)).toDeepEqual([
            {
                message: 'Interface field AnotherInterface.field expects type String but AnotherObject.field is type [String].',
                locations: [
                    { line: 7, column: 16 },
                    { line: 11, column: 16 },
                ],
            },
        ]);
    });
    (0, mocha_1.it)('accepts an Object with a subset non-null Interface field type', () => {
        const schema = (0, buildASTSchema_1.buildSchema)(`
      type Query {
        test: AnotherObject
      }

      interface AnotherInterface {
        field: String
      }

      type AnotherObject implements AnotherInterface {
        field: String!
      }
    `);
        (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(schema)).toDeepEqual([]);
    });
    (0, mocha_1.it)('rejects an Object with a superset nullable Interface field type', () => {
        const schema = (0, buildASTSchema_1.buildSchema)(`
      type Query {
        test: AnotherObject
      }

      interface AnotherInterface {
        field: String!
      }

      type AnotherObject implements AnotherInterface {
        field: String
      }
    `);
        (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(schema)).toDeepEqual([
            {
                message: 'Interface field AnotherInterface.field expects type String! but AnotherObject.field is type String.',
                locations: [
                    { line: 7, column: 16 },
                    { line: 11, column: 16 },
                ],
            },
        ]);
    });
    (0, mocha_1.it)('rejects an Object missing a transitive interface', () => {
        const schema = (0, buildASTSchema_1.buildSchema)(`
      type Query {
        test: AnotherObject
      }

      interface SuperInterface {
        field: String!
      }

      interface AnotherInterface implements SuperInterface {
        field: String!
      }

      type AnotherObject implements AnotherInterface {
        field: String!
      }
    `);
        (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(schema)).toDeepEqual([
            {
                message: 'Type AnotherObject must implement SuperInterface because it is implemented by AnotherInterface.',
                locations: [
                    { line: 10, column: 45 },
                    { line: 14, column: 37 },
                ],
            },
        ]);
    });
});
(0, mocha_1.describe)('Interfaces must adhere to Interface they implement', () => {
    (0, mocha_1.it)('accepts an Interface which implements an Interface', () => {
        const schema = (0, buildASTSchema_1.buildSchema)(`
      type Query {
        test: ChildInterface
      }

      interface ParentInterface {
        field(input: String): String
      }

      interface ChildInterface implements ParentInterface {
        field(input: String): String
      }
    `);
        (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(schema)).toDeepEqual([]);
    });
    (0, mocha_1.it)('accepts an Interface which implements an Interface along with more fields', () => {
        const schema = (0, buildASTSchema_1.buildSchema)(`
      type Query {
        test: ChildInterface
      }

      interface ParentInterface {
        field(input: String): String
      }

      interface ChildInterface implements ParentInterface {
        field(input: String): String
        anotherField: String
      }
    `);
        (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(schema)).toDeepEqual([]);
    });
    (0, mocha_1.it)('accepts an Interface which implements an Interface field along with additional optional arguments', () => {
        const schema = (0, buildASTSchema_1.buildSchema)(`
      type Query {
        test: ChildInterface
      }

      interface ParentInterface {
        field(input: String): String
      }

      interface ChildInterface implements ParentInterface {
        field(input: String, anotherInput: String): String
      }
    `);
        (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(schema)).toDeepEqual([]);
    });
    (0, mocha_1.it)('rejects an Interface missing an Interface field', () => {
        const schema = (0, buildASTSchema_1.buildSchema)(`
      type Query {
        test: ChildInterface
      }

      interface ParentInterface {
        field(input: String): String
      }

      interface ChildInterface implements ParentInterface {
        anotherField: String
      }
    `);
        (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(schema)).toDeepEqual([
            {
                message: 'Interface field ParentInterface.field expected but ChildInterface does not provide it.',
                locations: [
                    { line: 7, column: 9 },
                    { line: 10, column: 7 },
                ],
            },
        ]);
    });
    (0, mocha_1.it)('rejects an Interface with an incorrectly typed Interface field', () => {
        const schema = (0, buildASTSchema_1.buildSchema)(`
      type Query {
        test: ChildInterface
      }

      interface ParentInterface {
        field(input: String): String
      }

      interface ChildInterface implements ParentInterface {
        field(input: String): Int
      }
    `);
        (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(schema)).toDeepEqual([
            {
                message: 'Interface field ParentInterface.field expects type String but ChildInterface.field is type Int.',
                locations: [
                    { line: 7, column: 31 },
                    { line: 11, column: 31 },
                ],
            },
        ]);
    });
    (0, mocha_1.it)('rejects an Interface with a differently typed Interface field', () => {
        const schema = (0, buildASTSchema_1.buildSchema)(`
      type Query {
        test: ChildInterface
      }

      type A { foo: String }
      type B { foo: String }

      interface ParentInterface {
        field: A
      }

      interface ChildInterface implements ParentInterface {
        field: B
      }
    `);
        (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(schema)).toDeepEqual([
            {
                message: 'Interface field ParentInterface.field expects type A but ChildInterface.field is type B.',
                locations: [
                    { line: 10, column: 16 },
                    { line: 14, column: 16 },
                ],
            },
        ]);
    });
    (0, mocha_1.it)('accepts an Interface with a subtyped Interface field (interface)', () => {
        const schema = (0, buildASTSchema_1.buildSchema)(`
      type Query {
        test: ChildInterface
      }

      interface ParentInterface {
        field: ParentInterface
      }

      interface ChildInterface implements ParentInterface {
        field: ChildInterface
      }
    `);
        (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(schema)).toDeepEqual([]);
    });
    (0, mocha_1.it)('accepts an Interface with a subtyped Interface field (union)', () => {
        const schema = (0, buildASTSchema_1.buildSchema)(`
      type Query {
        test: ChildInterface
      }

      type SomeObject {
        field: String
      }

      union SomeUnionType = SomeObject

      interface ParentInterface {
        field: SomeUnionType
      }

      interface ChildInterface implements ParentInterface {
        field: SomeObject
      }
    `);
        (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(schema)).toDeepEqual([]);
    });
    (0, mocha_1.it)('rejects an Interface implementing a non-Interface type', () => {
        const schema = (0, buildASTSchema_1.buildSchema)(`
      type Query {
        field: String
      }

      input SomeInputObject {
        field: String
      }

      interface BadInterface implements SomeInputObject {
        field: String
      }
    `);
        (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(schema)).toDeepEqual([
            {
                message: 'Type BadInterface must only implement Interface types, it cannot implement SomeInputObject.',
                locations: [{ line: 10, column: 41 }],
            },
        ]);
    });
    (0, mocha_1.it)('rejects an Interface missing an Interface argument', () => {
        const schema = (0, buildASTSchema_1.buildSchema)(`
      type Query {
        test: ChildInterface
      }

      interface ParentInterface {
        field(input: String): String
      }

      interface ChildInterface implements ParentInterface {
        field: String
      }
    `);
        (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(schema)).toDeepEqual([
            {
                message: 'Interface field argument ParentInterface.field(input:) expected but ChildInterface.field does not provide it.',
                locations: [
                    { line: 7, column: 15 },
                    { line: 11, column: 9 },
                ],
            },
        ]);
    });
    (0, mocha_1.it)('rejects an Interface with an incorrectly typed Interface argument', () => {
        const schema = (0, buildASTSchema_1.buildSchema)(`
      type Query {
        test: ChildInterface
      }

      interface ParentInterface {
        field(input: String): String
      }

      interface ChildInterface implements ParentInterface {
        field(input: Int): String
      }
    `);
        (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(schema)).toDeepEqual([
            {
                message: 'Interface field argument ParentInterface.field(input:) expects type String but ChildInterface.field(input:) is type Int.',
                locations: [
                    { line: 7, column: 22 },
                    { line: 11, column: 22 },
                ],
            },
        ]);
    });
    (0, mocha_1.it)('rejects an Interface with both an incorrectly typed field and argument', () => {
        const schema = (0, buildASTSchema_1.buildSchema)(`
      type Query {
        test: ChildInterface
      }

      interface ParentInterface {
        field(input: String): String
      }

      interface ChildInterface implements ParentInterface {
        field(input: Int): Int
      }
    `);
        (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(schema)).toDeepEqual([
            {
                message: 'Interface field ParentInterface.field expects type String but ChildInterface.field is type Int.',
                locations: [
                    { line: 7, column: 31 },
                    { line: 11, column: 28 },
                ],
            },
            {
                message: 'Interface field argument ParentInterface.field(input:) expects type String but ChildInterface.field(input:) is type Int.',
                locations: [
                    { line: 7, column: 22 },
                    { line: 11, column: 22 },
                ],
            },
        ]);
    });
    (0, mocha_1.it)('rejects an Interface which implements an Interface field along with additional required arguments', () => {
        const schema = (0, buildASTSchema_1.buildSchema)(`
      type Query {
        test: ChildInterface
      }

      interface ParentInterface {
        field(baseArg: String): String
      }

      interface ChildInterface implements ParentInterface {
        field(
          baseArg: String,
          requiredArg: String!
          optionalArg1: String,
          optionalArg2: String = "",
        ): String
      }
    `);
        (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(schema)).toDeepEqual([
            {
                message: 'Object field ChildInterface.field includes required argument requiredArg that is missing from the Interface field ParentInterface.field.',
                locations: [
                    { line: 13, column: 11 },
                    { line: 7, column: 9 },
                ],
            },
        ]);
    });
    (0, mocha_1.it)('accepts an Interface with an equivalently wrapped Interface field type', () => {
        const schema = (0, buildASTSchema_1.buildSchema)(`
      type Query {
        test: ChildInterface
      }

      interface ParentInterface {
        field: [String]!
      }

      interface ChildInterface implements ParentInterface {
        field: [String]!
      }
    `);
        (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(schema)).toDeepEqual([]);
    });
    (0, mocha_1.it)('rejects an Interface with a non-list Interface field list type', () => {
        const schema = (0, buildASTSchema_1.buildSchema)(`
      type Query {
        test: ChildInterface
      }

      interface ParentInterface {
        field: [String]
      }

      interface ChildInterface implements ParentInterface {
        field: String
      }
    `);
        (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(schema)).toDeepEqual([
            {
                message: 'Interface field ParentInterface.field expects type [String] but ChildInterface.field is type String.',
                locations: [
                    { line: 7, column: 16 },
                    { line: 11, column: 16 },
                ],
            },
        ]);
    });
    (0, mocha_1.it)('rejects an Interface with a list Interface field non-list type', () => {
        const schema = (0, buildASTSchema_1.buildSchema)(`
      type Query {
        test: ChildInterface
      }

      interface ParentInterface {
        field: String
      }

      interface ChildInterface implements ParentInterface {
        field: [String]
      }
    `);
        (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(schema)).toDeepEqual([
            {
                message: 'Interface field ParentInterface.field expects type String but ChildInterface.field is type [String].',
                locations: [
                    { line: 7, column: 16 },
                    { line: 11, column: 16 },
                ],
            },
        ]);
    });
    (0, mocha_1.it)('accepts an Interface with a subset non-null Interface field type', () => {
        const schema = (0, buildASTSchema_1.buildSchema)(`
      type Query {
        test: ChildInterface
      }

      interface ParentInterface {
        field: String
      }

      interface ChildInterface implements ParentInterface {
        field: String!
      }
    `);
        (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(schema)).toDeepEqual([]);
    });
    (0, mocha_1.it)('rejects an Interface with a superset nullable Interface field type', () => {
        const schema = (0, buildASTSchema_1.buildSchema)(`
      type Query {
        test: ChildInterface
      }

      interface ParentInterface {
        field: String!
      }

      interface ChildInterface implements ParentInterface {
        field: String
      }
    `);
        (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(schema)).toDeepEqual([
            {
                message: 'Interface field ParentInterface.field expects type String! but ChildInterface.field is type String.',
                locations: [
                    { line: 7, column: 16 },
                    { line: 11, column: 16 },
                ],
            },
        ]);
    });
    (0, mocha_1.it)('rejects an Object missing a transitive interface', () => {
        const schema = (0, buildASTSchema_1.buildSchema)(`
      type Query {
        test: ChildInterface
      }

      interface SuperInterface {
        field: String!
      }

      interface ParentInterface implements SuperInterface {
        field: String!
      }

      interface ChildInterface implements ParentInterface {
        field: String!
      }
    `);
        (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(schema)).toDeepEqual([
            {
                message: 'Type ChildInterface must implement SuperInterface because it is implemented by ParentInterface.',
                locations: [
                    { line: 10, column: 44 },
                    { line: 14, column: 43 },
                ],
            },
        ]);
    });
    (0, mocha_1.it)('rejects a self reference interface', () => {
        const schema = (0, buildASTSchema_1.buildSchema)(`
      type Query {
        test: FooInterface
      }

      interface FooInterface implements FooInterface {
        field: String
      }
    `);
        (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(schema)).toDeepEqual([
            {
                message: 'Type FooInterface cannot implement itself because it would create a circular reference.',
                locations: [{ line: 6, column: 41 }],
            },
        ]);
    });
    (0, mocha_1.it)('rejects a circular Interface implementation', () => {
        const schema = (0, buildASTSchema_1.buildSchema)(`
      type Query {
        test: FooInterface
      }

      interface FooInterface implements BarInterface {
        field: String
      }

      interface BarInterface implements FooInterface {
        field: String
      }
    `);
        (0, expectJSON_1.expectJSON)((0, validate_1.validateSchema)(schema)).toDeepEqual([
            {
                message: 'Type FooInterface cannot implement BarInterface because it would create a circular reference.',
                locations: [
                    { line: 10, column: 41 },
                    { line: 6, column: 41 },
                ],
            },
            {
                message: 'Type BarInterface cannot implement FooInterface because it would create a circular reference.',
                locations: [
                    { line: 6, column: 41 },
                    { line: 10, column: 41 },
                ],
            },
        ]);
    });
});
(0, mocha_1.describe)('assertValidSchema', () => {
    (0, mocha_1.it)('does not throw on valid schemas', () => {
        const schema = (0, buildASTSchema_1.buildSchema)(`
      type Query {
        foo: String
      }
    `);
        (0, chai_1.expect)(() => (0, validate_1.assertValidSchema)(schema)).to.not.throw();
    });
    (0, mocha_1.it)('combines multiple errors', () => {
        const schema = (0, buildASTSchema_1.buildSchema)('type SomeType');
        (0, chai_1.expect)(() => (0, validate_1.assertValidSchema)(schema)).to.throw((0, dedent_1.dedent) `
      Query root type must be provided.

      Type SomeType must define one or more fields.`);
    });
});
