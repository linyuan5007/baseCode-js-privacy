...
function validateRootTypes(context) {
    ...
    if (!queryType) {
        ...
        context.reportError(`Query root type must be Object type, it cannot be ${(0, inspect_1.inspect)(queryType)}.`, getOperationTypeNode(schema, ast_1.OperationTypeNode.QUERY) ??
            queryType.astNode);
    }
    ...
    if (mutationType && !(0, definition_1.isObjectType)(mutationType)) {
        context.reportError('Mutation root type must be Object type if provided, it cannot be ' +
            `${(0, inspect_1.inspect)(mutationType)}.`, getOperationTypeNode(schema, ast_1.OperationTypeNode.MUTATION) ??
    ...
    if (subscriptionType && !(0, definition_1.isObjectType)(subscriptionType)) {
        context.reportError('Subscription root type must be Object type if provided, it cannot be ' +
            `${(0, inspect_1.inspect)(subscriptionType)}.`, getOperationTypeNode(schema, ast_1.OperationTypeNode.SUBSCRIPTION) ??
...
function validateDirectives(context) {
    for (const directive of context.schema.getDirectives()) {
        ...
        if (!(0, directives_1.isDirective)(directive)) {
            context.reportError(`Expected directive but got: ${(0, inspect_1.inspect)(directive)}.`, directive?.astNode);
            continue;
        }
        ...
        for (const arg of directive.args) {
            ...
            if ((0, definition_1.isRequiredArgument)(arg) && arg.deprecationReason != null) {
                context.reportError(`Required argument @${directive.name}(${arg.name}:) cannot be deprecated.`, [getDeprecatedDirectiveNode(arg.astNode), arg.astNode?.type]);
            }
        }
    }
}
    ...
    for (const field of fields) {
        ...
        if (!(0, definition_1.isOutputType)(field.type)) {
            context.reportError(`The type of ${type.name}.${field.name} must be Output Type ` +
                `but got: ${(0, inspect_1.inspect)(field.type)}.`, field.astNode?.type);
        }
        // Ensure the arguments are valid
        for (const arg of field.args) {
            ...
            if (!(0, definition_1.isInputType)(arg.type)) {
                context.reportError(`The type of ${type.name}.${field.name}(${argName}:) must be Input ` +
                    `Type but got: ${(0, inspect_1.inspect)(arg.type)}.`, arg.astNode?.type);
            }
            if ((0, definition_1.isRequiredArgument)(arg) && arg.deprecationReason != null) {
                context.reportError(`Required argument ${type.name}.${field.name}(${argName}:) cannot be deprecated.`, [getDeprecatedDirectiveNode(arg.astNode), arg.astNode?.type]);
            }
        }
    }
}
    ...
    for (const ifaceField of Object.values(iface.getFields())) {
        ...
        if (!(0, typeComparators_1.isTypeSubTypeOf)(context.schema, typeField.type, ifaceField.type)) {
            context.reportError(`Interface field ${iface.name}.${fieldName} expects type ` +
                ...
                `is type ${(0, inspect_1.inspect)(typeField.type)}.`, [ifaceField.astNode?.type, typeField.astNode?.type]);
        }
        ...
        for (const ifaceArg of ifaceField.args) {
            ...
            if (!(0, typeComparators_1.isEqualType)(ifaceArg.type, typeArg.type)) {
                context.reportError(`Interface field argument ${iface.name}.${fieldName}(${argName}:) ` +
                    ...
                    `${(0, inspect_1.inspect)(typeArg.type)}.`, [ifaceArg.astNode?.type, typeArg.astNode?.type]);
            }
...
function validateOneOfInputObjectField(type, field, context) {
    if ((0, definition_1.isNonNullType)(field.type)) {
        context.reportError(`OneOf input field ${type.name}.${field.name} must be nullable.`, field.astNode?.type);
    }
...
function getDeprecatedDirectiveNode(definitionNode) {
    return definitionNode?.directives?.find((node) => node.name.value === directives_1.GraphQLDeprecatedDirective.name);
}
