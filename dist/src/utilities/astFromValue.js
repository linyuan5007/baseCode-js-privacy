...
function astFromValue(value, type) {
    if ((0, definition_1.isNonNullType)(type)) {
        ...
        if (astValue?.kind === kinds_1.Kind.NULL) {
            return null;
        }
        return astValue;
    }
    ...
    if ((0, definition_1.isListType)(type)) {
        ...
        if ((0, isIterableObject_1.isIterableObject)(value)) {
            const valuesNodes = [];
            ...
            return { kind: kinds_1.Kind.LIST, values: valuesNodes };
        }
        ...
        for (const field of Object.values(type.getFields())) {
            ...
            if (fieldValue) {
                fieldNodes.push({
                    ...
                    name: { kind: kinds_1.Kind.NAME, value: field.name },
                    value: fieldValue,
                });
            }
        }
        return { kind: kinds_1.Kind.OBJECT, fields: fieldNodes };
    }
    if ((0, definition_1.isLeafType)(type)) {
        ...
        if (typeof serialized === 'boolean') {
            return { kind: kinds_1.Kind.BOOLEAN, value: serialized };
        }
        // JavaScript numbers can be Int or Float values.
        if (typeof serialized === 'number' && Number.isFinite(serialized)) {
            const stringNum = String(serialized);
            return integerStringRegExp.test(stringNum)
                ? { kind: kinds_1.Kind.INT, value: stringNum }
                : { kind: kinds_1.Kind.FLOAT, value: stringNum };
        }
        if (typeof serialized === 'string') {
            // Enum types use Enum literals.
            if ((0, definition_1.isEnumType)(type)) {
                return { kind: kinds_1.Kind.ENUM, value: serialized };
            }
            // ID types can use Int literals.
            if (type === scalars_1.GraphQLID && integerStringRegExp.test(serialized)) {
                return { kind: kinds_1.Kind.INT, value: serialized };
            }
            return {
        ...
        throw new TypeError(`Cannot convert value to AST: ${(0, inspect_1.inspect)(serialized)}.`);
    }
    /* c8 ignore next 3 */
