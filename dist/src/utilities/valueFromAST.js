...
function valueFromAST(valueNode, type, variables) {
    ...
    if (valueNode.kind === kinds_1.Kind.VARIABLE) {
        ...
        if (variableValue === null && (0, definition_1.isNonNullType)(type)) {
    ...
    if ((0, definition_1.isNonNullType)(type)) {
        if (valueNode.kind === kinds_1.Kind.NULL) {
            return; // Invalid: intentionally return no value.
        }
    ...
    if ((0, definition_1.isListType)(type)) {
        ...
        if (valueNode.kind === kinds_1.Kind.LIST) {
            const coercedValues = [];
            for (const itemNode of valueNode.values) {
                if (isMissingVariable(itemNode, variables)) {
                    // If an array contains a missing variable, it is either coerced to
                    // null or if the item type is non-null, it considered invalid.
                    if ((0, definition_1.isNonNullType)(itemType)) {
                        return; // Invalid: intentionally return no value.
                    }
    ...
    if ((0, definition_1.isInputObjectType)(type)) {
        if (valueNode.kind !== kinds_1.Kind.OBJECT) {
            return; // Invalid: intentionally return no value.
        }
        const coercedObj = Object.create(null);
        const fieldNodes = (0, keyMap_1.keyMap)(valueNode.fields, (field) => field.name.value);
...
function isMissingVariable(valueNode, variables) {
    return (valueNode.kind === kinds_1.Kind.VARIABLE &&
