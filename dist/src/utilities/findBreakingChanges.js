...
function findDirectiveChanges(oldSchema, newSchema) {
    ...
    for (const [oldDirective, newDirective] of directivesDiff.persisted) {
        ...
        for (const newArg of argsDiff.added) {
            if ((0, definition_1.isRequiredArgument)(newArg)) {
                schemaChanges.push({
...
function findTypeChanges(oldSchema, newSchema) {
    ...
    for (const oldType of typesDiff.removed) {
        schemaChanges.push({
            ...
            description: (0, scalars_1.isSpecifiedScalarType)(oldType)
                ? `Standard scalar ${oldType.name} was removed because it is not referenced anymore.`
                : `${oldType.name} was removed.`,
        });
    }
    for (const [oldType, newType] of typesDiff.persisted) {
        if ((0, definition_1.isEnumType)(oldType) && (0, definition_1.isEnumType)(newType)) {
            schemaChanges.push(...findEnumTypeChanges(oldType, newType));
        }
        else if ((0, definition_1.isUnionType)(oldType) && (0, definition_1.isUnionType)(newType)) {
            schemaChanges.push(...findUnionTypeChanges(oldType, newType));
        }
        else if ((0, definition_1.isInputObjectType)(oldType) && (0, definition_1.isInputObjectType)(newType)) {
            schemaChanges.push(...findInputObjectTypeChanges(oldType, newType));
        }
        else if ((0, definition_1.isObjectType)(oldType) && (0, definition_1.isObjectType)(newType)) {
            schemaChanges.push(...findFieldChanges(oldType, newType), ...findImplementedInterfacesChanges(oldType, newType));
        }
        else if ((0, definition_1.isInterfaceType)(oldType) && (0, definition_1.isInterfaceType)(newType)) {
            schemaChanges.push(...findFieldChanges(oldType, newType), ...findImplementedInterfacesChanges(oldType, newType));
        }
...
function findInputObjectTypeChanges(oldType, newType) {
    ...
    for (const newField of fieldsDiff.added) {
        if ((0, definition_1.isRequiredInputField)(newField)) {
            schemaChanges.push({
    ...
    for (const [oldArg, newArg] of argsDiff.persisted) {
        ...
        if (!isSafe) {
            schemaChanges.push({
                ...
                description: `${oldType.name}.${oldField.name} arg ${oldArg.name} has changed type from ` +
...
function isChangeSafeForObjectOrInterfaceField(oldType, newType) {
    if ((0, definition_1.isListType)(oldType)) {
        return (
        // if they're both lists, make sure the underlying types are compatible
        ((0, definition_1.isListType)(newType) &&
            isChangeSafeForObjectOrInterfaceField(oldType.ofType, newType.ofType)) ||
            // moving from nullable to non-null of the same underlying type is safe
            ((0, definition_1.isNonNullType)(newType) &&
                isChangeSafeForObjectOrInterfaceField(oldType, newType.ofType)));
    }
    if ((0, definition_1.isNonNullType)(oldType)) {
        // if they're both non-null, make sure the underlying types are compatible
        return ((0, definition_1.isNonNullType)(newType) &&
            isChangeSafeForObjectOrInterfaceField(oldType.ofType, newType.ofType));
    }
    return (
    // if they're both named types, see if their names are equivalent
    ((0, definition_1.isNamedType)(newType) && oldType.name === newType.name) ||
        // moving from nullable to non-null of the same underlying type is safe
        ((0, definition_1.isNonNullType)(newType) &&
            isChangeSafeForObjectOrInterfaceField(oldType, newType.ofType)));
}
function isChangeSafeForInputObjectFieldOrFieldArg(oldType, newType) {
    if ((0, definition_1.isListType)(oldType)) {
        // if they're both lists, make sure the underlying types are compatible
        return ((0, definition_1.isListType)(newType) &&
    ...
    // if they're both named types, see if their names are equivalent
    return (0, definition_1.isNamedType)(newType) && oldType.name === newType.name;
}
...
function stringifyValue(value, type) {
    const ast = (0, astFromValue_1.astFromValue)(value, type);
    (0, invariant_1.invariant)(ast != null);
    return (0, printer_1.print)((0, sortValueNode_1.sortValueNode)(ast));
}
function diff(oldArray, newArray) {
    const added = [];
    const removed = [];
    const persisted = [];
    const oldMap = (0, keyMap_1.keyMap)(oldArray, ({ name }) => name);
    const newMap = (0, keyMap_1.keyMap)(newArray, ({ name }) => name);
