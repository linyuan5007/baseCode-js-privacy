...
function coerceInputValueImpl(inputValue, type, onError, path) {
    ...
    if ((0, definition_1.isInputObjectType)(type)) {
        if (!(0, isObjectLike_1.isObjectLike)(inputValue) || Array.isArray(inputValue)) {
        ...
        if (type.isOneOf) {
            ...
            if (keys.length !== 1) {
                onError((0, Path_1.pathToArray)(path), inputValue, new GraphQLError_1.GraphQLError(`Exactly one key must be specified for OneOf type "${type.name}".`));
            }
            const key = keys[0];
            const value = coercedValue[key];
            if (value === null) {
                onError((0, Path_1.pathToArray)(path).concat(key), value, new GraphQLError_1.GraphQLError(`Field "${key}" must be non-null.`));
            }
        }
        return coercedValue;
    }
